...
to $f$, and others that use higher derivatives, notably the Hessian.
...
and physics. Another useful process is the Ornstein-Uhlenbeck process: 
...
a limit, i.e., 
\[
\nabla f(x)^{\top}y=\lim_{h\rightarrow0}\frac{(f(x)+h\nabla f(x)^{\top}y+\frac{1}{2}h^{2}...)-f(x)}{h}=\lim_{h\rightarrow0}\frac{h\nabla f(x)^{\top}y+\frac{1}{2}h^{2}...}{h}
\]
and only the first term survives, as the second and later terms go
...
\[
[x]_{t}=\lim_{n\to\infty}\sum^{n}_{i=1}(x_{t_{i}}-x_{t_{i-1}})^{2}=\int^{t}_{0}\sigma^{2}_{s}\,ds,
\]
...
\[
[x]_{t}=\int^{t}_{0}\sigma_{s}\sigma^{\T}_{s}\,ds\,
\]
and $d[X^{i},X^{j}]_{t}=(\sigma_{t}\sigma^{\top}_{t})_{ij}dt$. We
...
$\sigma(x_{t})\in\R^{n\times m}$, and $x_{0}$ is drawn from $p_{0}$.
Then the density $p_{t}$ of $x_{t}$ satisfies the equation
\[
...
\]
Taking derivatives on both sides with respect to $t$, using It\^{o}'s
lemma (Lemma \ref{lem:itos}), and noting that $\E dW_{t}=0$, we
have that
\begin{align*}
\int\phi(x)\frac{dp_{t}}{dt}dx & =\E\left(\nabla\phi(x_{t})^{\top}\mu(x_{t})+\frac{1}{2}\tr(\sigma(x_{t})^{\top}\nabla^{2}\phi(x_{t})\sigma(x_{t}))\right)\\
 & =\E\left(\nabla\phi(x_{t})^{\top}\mu(x_{t})+\frac{1}{2}\tr(\nabla^{2}\phi(x_{t})D(x_{t}))\right).
\end{align*}
...
density proportional to $e^{-f}$ is stationary for the Langevin dynamics.
\end{thm}
...
\begin{equation}
\frac{dp_{t}}{dt}=\sum_{i}\frac{\partial}{\partial x_{i}}(\frac{\partial f(x)}{\partial x_{i}}p_{t}(x))+\sum_{i}\frac{\partial^{2}}{\partial x^{2}_{i}}\left[p_{t}(x)\right].\label{eq:dp_LD}
\end{equation}
To check stationarity, set $p_{t}$ proportional to $e^{-f}$ in the
right-hand side; then the LHS is zero and we can rewrite the above
as
\begin{align*}
...
(Hint: take expectations on both sides of It\^{o}'s lemma for appropriate
$f(X_{t})$, and use $\E df(X_{t})=d\E f(X_{t})$ when $f$ is sufficiently
smooth). 
\end{xca}
...
there is a coupling $\gamma$ between $x_{t}$ and $y_{t}$ such that
\[
...
fundamental functional constants of the target density, which we define
next. These constants will play an important role in later algorithms
and analysis, and we will also see equivalent geometric formulations
of them.
\begin{defn}
...
to the density $\nu(x)\propto e^{-f(x)}$ with decay rate $2/C_{LSI}$,
...
\begin{proof}
We use the previous two lemmas to obtain
\[
...
\end{proof}
\begin{xca}
Show that Langevin dynamics applied to a target density $\nu$ with
Poincar\'e constant $C_{PI}$ satisfies 
\[
...
\]
Since we are interested only in vector fields that generate minimal
movement in Wasserstein distance, we consider the optimization problem
\[
\min_{-\nabla\cdot(pc)=v}\frac{1}{2}\int p(x)\|c(x)\|^{2}dx
\]
where we can think of $v$ as the change in $p_{t}$. Let $\lambda(x)$
be the Lagrange multiplier of the constraint $-\nabla\cdot(pc)=v$.
...
c(x)=-\nabla\lambda(x).
\]
This proves that any vector field that generates minimal movement
in Wasserstein distance is a gradient field. Also, we have that $v(x)=\nabla\cdot(p(x)\nabla\lambda(x)).$
Note that the right-hand side is an elliptic differential equation
and hence for any $v$ with $\int v(x)dx=0$, there is a unique solution
$\lambda(x)$ up to an additive constant. Therefore, we can write
$v(x)=\nabla\cdot(p(x)\nabla\lambda(x))$ for some $\lambda(x)$.

Next, we note that the movement is given by
\[
\|v\|^{2}_{p}=\int p(x)\|c(x)\|^{2}dx=\E_{x\sim p}\|\nabla\lambda(x)\|^{2}.
\]
\end{proof}
...
Then, we have that
\[
\frac{d\rho}{dt}=\argmin_{v\in T_{\rho}P_{2}(\Rn)}\left\langle \nabla F(\rho),v\right\rangle _{\rho}+\frac{1}{2}\|v\|^{2}_{\rho}.
\]
...
Dominance'':
\begin{equation}
\norm{\nabla F(x)}^{2}_{x}\ge\alpha\cdot(F(x)-\min_{y}F(y))\quad\text{for all }x\label{eq:LD_strong_convexity}
\end{equation}
...
We write
\[
\frac{d}{dt}(F(x_{t})-\min_{y}F(y))=\langle\nabla F(x_{t}),\frac{dx_{t}}{dt}\rangle_{x_{t}}=-\|\nabla F(x_{t})\|^{2}_{x_{t}}\leq-\alpha(F(x_{t})-\min_{y}F(y)).
\]
...
\[
\Ent_{\nu}(g^{2})\leq2C_{LSI}\int\norm{\nabla g}^{2}\,d\nu
\]
 implies the condition (\ref{eq:LD_strong_convexity}) with $\alpha=2/C_{LSI}$.
\end{lem}
...
we obtain another proof of Theorem \ref{thm:LDinKL}.
...
needs more assumptions, and there is considerable flexibility in choosing
discretizations. This is similar to the situation with gradient descent
for optimization. As we saw in Section \ref{sec:LD_GD}, it turns
out that Langevin dynamics is in fact gradient descent in the space
of probability measures under the Wasserstein metric, where the function
being minimized is the KL-divergence of the current density from the
target stationary density. For more on this view of sampling as optimization
over measures, see \cite{wibisono2018sampling}. See \cite{lee2024eldan}
for tight log-Sobolev estimates for isotropic logconcave measures
in terms of the support diameter.
