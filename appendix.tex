...
is given by
\[
\nabla\cdot v=\sum^{n}_{i=1}\frac{\partial v_{i}}{\partial x_{i}}.
\]

\begin{example}
Consider $v(x)=(2x_{1}+x^{3}_{3},3x_{1}\sin(x_{2}),4x_{3}x^{2}_{1})$.
Then $\nabla\cdot v=2+3x_{1}\cos(x_{2})+4x^{2}_{1}$. 
\end{example}
...
\]
And using the divergence theorem, namely, $\int_{\Omega}\nabla\cdot w=\int_{\partial\Omega}w\cdot\overrightarrow{n}$,
where $\overrightarrow{n}$ is the outward normal vector at a point
in $\partial\Omega$, we have:
\[
\int_{\Omega}\nabla f\cdot v=\int_{\partial\Omega}fv\cdot\overrightarrow{n}-\int_{\Omega}f(\nabla\cdot v).
\]
The first term on the right-hand side becomes zero if the vector field
$v$ goes to $0$ at the boundary of the domain (e.g., if the domain
is $\R^{n}$, then if it decays at infinity). 
...
Here, we give a few examples for this. We define $Df(x)[h]$ to be
the directional derivative of $f$ at $x$ along the direction $h$.
Namely, 
\[
...
$y$ axis. Suppose that this is the time-minimizing curve for a bead
to slide frictionlessly from $(0,0)$ to $(1,-1)$ under uniform gravity.
Then, we have that
\[
2uu''+(u')^{2}+1=0.
\]
...
\end{rem*}
\begin{figure}
\centering{}\includegraphics[width=4in]{Figures/fig3}\caption{The ``fastest'' curve\label{fig:rel-1}}
\end{figure}

\begin{proof}
Given a curve $u=u(x)$, the total travel time is
\[
T(u)=\int^{1}_{0}\frac{ds(x)}{v(x)}=\int^{1}_{0}\frac{\sqrt{1+(u'(x))^{2}}}{v(x)}dx
\]
...
\[
T(u)=\int^{1}_{0}\sqrt{\frac{1+(u'(x))^{2}}{-2gu(x)}}dx.
\]
Since $u$ is a \emph{time-minimizing} curve, any local change in
$u$ cannot reduce the time, i.e., 
\[
DT(u)[h]=0
\]
...
\begin{align*}
DT(u)[h] & =\int^{1}_{0}-\frac{1}{2}\frac{\sqrt{1+u'(x)^{2}}}{\sqrt{-2g}u(x)^{3/2}}\frac{d}{dt}u(x)dx+\int^{1}_{0}\frac{1}{2}\frac{2u'(x)}{\sqrt{-2gu(x)}\sqrt{1+u'(x)^{2}}}\frac{d}{dt}u'(x)dx.\\
 & =\int^{1}_{0}-\frac{1}{2}\frac{\sqrt{1+u'(x)^{2}}}{\sqrt{-2gu(x)}u(x)}h(x)dx+\int^{1}_{0}\frac{u'(x)h'(x)}{\sqrt{-2gu(x)}\sqrt{1+u'(x)^{2}}}dx.
\end{align*}
Note that the second term involves $h'(x)$. To change the term $h'(x)$
to $h(x)$, we use the integration by parts (with respect to $x$,
not $t$!):
\[
\int^{1}_{0}\frac{u'(x)h'(x)}{\sqrt{-2gu(x)}\sqrt{1+(u'(x))^{2}}}dx=\left[\frac{u'(x)h(x)}{\sqrt{-2gu(x)}\sqrt{1+(u'(x))^{2}}}\right]^{1}_{0}-\int^{1}_{0}\frac{d}{dx}\left(\frac{u'(x)}{\sqrt{-2gu(x)}\sqrt{1+(u'(x))^{2}}}\right)h(x)dx.
\]
Since the endpoints of the curve are fixed, we have $h(1)=h(0)=0$.
Hence, the first term on the right-hand side is $0$. Continuing,
\begin{align*}
DT(u)[h]= & \int^{1}_{0}-\frac{1}{2}\frac{\sqrt{1+u'(x)^{2}}}{\sqrt{-2gu(x)}u(x)}h(x)dx-\int^{1}_{0}\frac{d}{dx}\left(\frac{u'(x)}{\sqrt{-2gu(x)}\sqrt{1+u'(x)^{2}}}\right)h(x)dx\\
= & \int^{1}_{0}-\frac{1}{2}\frac{\sqrt{1+(u'(x))^{2}}}{\sqrt{-2gu(x)}u(x)}h(x)dx-\int^{1}_{0}\frac{u''(x)}{\sqrt{-2gu(x)}\sqrt{1+u'(x)^{2}}}h(x)dx\\
 & +\int^{1}_{0}\frac{1}{2}\frac{u'(x)^{2}}{\sqrt{-2gu(x)}u(x)\sqrt{1+u'(x)^{2}}}h(x)dx\\
 & +\int^{1}_{0}\frac{u'(x)u'(x)u''(x)}{\sqrt{-2gu(x)}(1+u'(x){}^{2})^{3/2}}h(x)dx.
\end{align*}
Hence, we have $DT(u)[h]=\int^{1}_{0}a(x)h(x)dx$ where
\begin{align}
...
\]
where $D_{x}f$ is the Jacobian of $f$ with respect to the $x$-variables
and $Dg(x)$ is the Jacobian of $g$. We note that the formula can
be obtained by taking derivatives on both sides with respect to $x$.
Sometimes, taking derivatives on both sides can greatly simplify calculations.
Here are some examples.
\begin{lem}
Consider $x_{t}=\argmin_{x\in\Rn}f_{t}(x)$ where $f_{t}$ are strictly
convex and $\nabla^{2}f_{t}(x_{t})$ is positive definite. Then, we
have
\[
...
\]
Since $\nabla^{2}f_{t}(x_{t})$ is positive definite and is invertible.
...
Often, one would like to compute the differential of a quantity that
is most appropriately expressed by an integral, and under mild assumptions
we can do so by simply swapping the integral and derivative, i.e.
for time-differentiable and (Lebesgue) integrable $f(x,t)$ we want
to justify
\[
...
we can perform the above trick as long as there exists an integrable
...
In this section, we introduce the KKT conditions and show how to use
them to solve optimization problems by hand.
...
$f$, $h_{i}$ and $\ell_{j}$. If $x$ is a local minimum and the
gradients of the active inequalities together with the equality gradients
are linearly independent at $x$, then there exist multipliers $u_{i}$
and $v_{j}$ such that $x$ satisfies the KKT conditions:
\begin{itemize}
...
We prove Hölder's inequality as an example:
\begin{fact}
...
\begin{proof}
Assume first that $1<p,q<\infty$. By symmetry, we may assume $y_{i}\ge0$
and take a nonnegative maximizer; it suffices to compute
\[
...
h(x)\leq0, & u\geq0.
\end{align*}
Note that $\nabla f(x)=-y$ and $(\nabla h(x))_{i}=\|x\|^{1-p}_{p}x^{p-1}_{i}$.
From the stationary condition, we have
\[
y=u\|x\|^{1-p}_{p}\cdot x^{p-1}.
\]
...
y=u\cdot x^{p-1}.
\]
Hence, we have $1=\sum_{i}x^{p}_{i}=\sum_{i}(y_{i}/u)^{\frac{p}{p-1}}=\sum_{i}(y_{i}/u)^{q}$.
Hence, we have $u=\|y\|_{q}$ 

Now, we can compute $\sum_{i}x_{i}y_{i}$ as follows
\[
\sum_{i}x_{i}y_{i}=\sum_{i}(\frac{y_{i}}{u})^{\frac{1}{p-1}}y_{i}=\frac{1}{u^{1/(p-1)}}\sum_{i}y^{p/(p-1)}_{i}=\|y\|_{q}.
\]
The endpoint cases are direct: if $p=1,q=\infty$, then $\sum_{i}|x_{i}y_{i}|\leq\|y\|_{\infty}\|x\|_{1}$,
and the case $p=\infty,q=1$ is symmetric.
\end{proof}
