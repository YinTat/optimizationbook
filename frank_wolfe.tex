...
\textbf{Input:} Initial point $x^{(0)}\in\mathcal{D}$.
...
a constant $C_{f}$ such that for any $x,y\in\mathcal{D}$ and $h\in[0,1]$,
\[
f((1-h)x+hy)\leq f(x)+h\left\langle \nabla f(x),y-x\right\rangle +\frac{1}{2}C_{f}h^{2}.
\]
Then, for every $k\ge1$, we have
\[
...
\[
f(x^{(k+1)})\leq f(x^{(k)})+h_{k}\left\langle \nabla f(x^{(k)}),y^{(k)}-x^{(k)}\right\rangle +\frac{1}{2}C_{f}h^{2}_{k}.
\]
Note that 
\[
...
\[
f(x^{(k+1)})\leq f(x^{(k)})-h_{k}(f(x^{(k)})-f(x^{*}))+\frac{1}{2}C_{f}h^{2}_{k}.
\]
Let $e_{k}=f(x^{(k)})-f(x^{*})$. Then, 
\[
e_{k+1}\leq(1-h_{k})e_{k}+\frac{1}{2}C_{f}h^{2}_{k}.
\]
Since $h_{0}=1$, the recurrence gives $e_{1}\leq\frac{1}{2}C_{f}\leq\frac{2}{3}C_{f}$.
By induction, for every $k\ge1$, we have that $e_{k}\leq\frac{2C_{f}}{k+2}$.
\end{proof}
\begin{rem*}
