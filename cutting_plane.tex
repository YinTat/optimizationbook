...
and were developed into polynomial-time algorithms by \cite{khachiyan1980polynomial,grotschel1981ellipsoid,bertsimas2004solving}.
...
$E^{(k)}$ and query the oracle for $g(x^{(k)})$. The guarantee for
$g(.)$ implies that $K$ lies in the halfspace 
\[
...
Find $E^{(k+1)}\supseteq E^{(k)}\cap H^{(k)}$ where
\begin{equation}
...
positive definite. After we compute $g(x^{(k)})$ and $H^{(k)}$ via
(\ref{eq:Hk}), we define $E^{(k+1)}$ to be the smallest volume ellipsoid
containing $E^{(k)}\cap H^{(k)}$. The key observation is that the
volume of the ellipsoid $E^{(k)}$ decreases by a factor of $1-\Theta(\frac{1}{n})$
...
Therefore, the ellipsoid method does not run faster even for simple
functions, e.g., linear functions, making it provably ``slow'' in
practice.
\end{rem}
...
and $g(x^{(k)})=e_{1}$. Therefore $H^{(k)}$ is simply the halfspace
...
Note that since $g(x^{(k)})=e_{1},$ we have $A^{(k+1)}=\frac{n^{2}}{n^{2}-1}\left(I-\frac{2}{n+1}e_{1}e^{\top}_{1}\right)$.
Therefore, we have that
\begin{align*}
\left(\frac{\vol E^{(k+1)}}{\vol E^{(k)}}\right)^{2}=\left|\frac{\det A^{(k+1)}}{\det A^{(k)}}\right| & =\left(\frac{n^{2}}{n^{2}-1}\right)^{n}\det\left(I-\frac{2}{n+1}e_{1}e^{\top}_{1}\right)\\
...
\[
\left(I-\frac{2}{n+1}e_{1}e^{\top}_{1}\right)^{-1}=\left(I+\frac{2}{n-1}e_{1}e^{\top}_{1}\right)
\]
we have
\begin{align*}
 & (x+\frac{e_{1}}{n+1})^{\top}\left(1-\frac{1}{n^{2}}\right)\left(I+\frac{2}{n-1}e_{1}e^{\top}_{1}\right)(x+\frac{e_{1}}{n+1})\\
...
Let $R\defeq\max_{x\in P}\norm x_{2}$ be an outer radius for $P$
and let its volume radius be $r=\vol(P)^{1/n}$. Then, we can find
$x\in P$ for which 
\[
...
0 & \text{if }a^{\top}_{i}x\geq b_{i}\text{ for all }i\\
+\infty & \text{otherwise.}
\end{cases}.\label{eq:LP_func}
\end{equation}

For this function $L$, we can use the separation oracle $v(x)=c$
if $Ax\geq b$ and $v(x)=-a_{i}$ if $a^{\top}_{i}x<b_{i}$. If there
are many violated constraints, any one of them will do.

In this case, we can set $\Omega=P$. We can simply pick $E^{(0)}$
to be the Euclidean ball centered at $0$ with radius $R$. We apply
Theorem \ref{thm:ellipsoid} to find $x$ such that
\[
c^{\top}x-\min_{y\in P}c^{\top}y\leq\varepsilon\cdot(\max_{y\in P}c^{\top}y-\min_{y\in P}c^{\top}y)
\]
...
The randomized center of gravity method is defined as follows: 
...
namely $\E_{x\sim p}x=0$ and $\E_{x\sim p}xx^{\top}=I$. For any
unit vector $\theta\in\R^{n}$, $t\in\R$ we have
\[
...
\[
\int^{M}_{-M}t\frac{dP(t)}{dt}\,dt=0
\]
Integration by parts gives that $\int^{M}_{-M}P(t)dt=M$. Note that
$P(t)$ is increasing logconcave, if $P(0)$ is too small, it would
make $\int^{M}_{-M}P(t)dt$ too small. To be precise, since $P$ is
...
Hence,
\[
M=\int^{M}_{-M}P(t)dt\leq\int^{\frac{1}{\alpha}}_{-\infty}P(0)e^{\alpha t}dt+\int^{M}_{1/\alpha}1dt=\frac{eP(0)}{\alpha}+M-\frac{1}{\alpha}.
\]
This shows that $P(0)\geq\frac{1}{e}$. Applying the same argument
with $-\theta$ gives $\P_{x\sim p}(x^{\top}\theta\ge0)\ge1/e$.

Next, Lemma \ref{lem:logcon_max-1} shows that $\max_{x}p(x)\le1$.
Therefore, the cumulative distribution $P$ is $1$-Lipschitz. Hence,
for $t\ge0$ we have
\[
\P_{x\sim p}(x^{\top}\theta\geq t)\ge\P_{x\sim p}(x^{\top}\theta\geq0)-t\geq\frac{1}{e}-t.
\]
For $t<0$, the claim follows from $\P_{x\sim p}(x^{\top}\theta\ge t)\ge\P_{x\sim p}(x^{\top}\theta\ge0)\ge1/e\ge1/e-\left|t\right|$.
\end{proof}
\begin{lem}
...
from a convex body $K$ and $y=\frac{1}{N}\sum^{N}_{i=1}y^{(i)}$.
...
\begin{align*}
\E\norm y^{2} & =\frac{1}{N^{2}}\E\sum^{N}_{i=1}\norm{y^{(i)}}^{2}=\frac{1}{N}\E\norm{y^{(1)}}^{2}=\frac{n}{N}.
\end{align*}
Therefore, by Jensen's inequality,
\[
\E\norm y\le\sqrt{\E\left(\norm y^{2}\right)}=\sqrt{\frac{n}{N}}.
\]
Thus, we can apply Theorem \ref{thm:grun-1} with $t=|\theta^{\top}y|$
and then use $\E|\theta^{\top}y|\le\E\norm y\le\sqrt{\frac{n}{N}}$
...
\centering{}\includegraphics[width=0.85\columnwidth]{Figures/Thrm_3\lyxdot 20}\caption{Affine transformations for Center of Gravity method}
\end{figure}
...
length $\epsilon$. Consider the oracle that always returns an axis-parallel
halfspace that does not cut any little cube and contains at least
half of the volume of the ``remaining'' region, i.e., the set given
by the original cube intersected with all halfspaces given by the
oracle so far. This is always possible since for any halfspace either
the halfspace or its complement will contain at least half the volume
of any set. Thus each query at best halves the remaining volume. To
solve the problem, the algorithm needs to cut down to a set of volume
$\epsilon^{n}$ starting from a set of volume $R^{n}.$ Thus it needs
at least $n\log_{2}(R/\epsilon)$ queries.
\end{proof}
\begin{figure}[h]
