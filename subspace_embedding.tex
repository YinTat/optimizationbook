...
When $n=d=1$, $\|\Pi x\|^{2}$ is $1/s$ times the number of nonzero
entries in its single column. Therefore, we indeed need that $s$
scales like $1/\epsilon^{2}$. The advantage of a sparse embedding
is that applying it can be much more efficient.
\begin{rem}
...
\begin{lem}
Assume that $s\gg\frac{m}{n}\log(\frac{1}{\delta}),$$s\gg\log^{2}(\frac{1}{\delta})/\epsilon^{2}$
