...
\]
Continuing similarly, we have $x=\sum^{\infty}_{i=1}t_{i}x_{i}$ with
$0\leq t_{i}\leq\frac{1}{2^{i-1}}$. Hence, we have that
\begin{align*}
x^{\top}(U^{\top}\Pi^{\top}\Pi U-I_{d})x & =\sum_{i,j}t_{i}t_{j}x^{\top}_{i}(U^{\top}\Pi^{\top}\Pi U-I_{d})x_{j}\\
...
\[
(1-O(\varepsilon))I\preceq\sum^{m}_{r=1}\pi^{\top}_{r}UU^{\top}\pi_{r}\preceq(1+O(\varepsilon))I.
\]
\end{proof}
