...
theory. Suppose, for a moment, that there were a polynomial $q(x)$
such that $q(0)=1$ and $q(x)=0$ for all $x>0$. By assumption, we
know that the constant term in $q$ is $1$ and hence we can write
\[
q(x)=1-xp(x)
\]
...
computing $p(A)b$ which takes $\text{deg}(p)\cdot\nnz(A)$ time.
Unfortunately, there is no such polynomial $q$. In this section,
we will discuss whether there is a polynomial that approximately satisfies
the condition above. 
...
shows that it requires degree $O(\frac{L}{\mu}\log(\frac{1}{\epsilon}))$
to bound the error by $\epsilon$. The key fact we will use is that
we can approximate the Richardson polynomial above, which has degree
$k$, by a polynomial of degree $\tilde{O}(\sqrt{k})$. The construction
...
\begin{defn}
For any integer $d$, the Chebyshev polynomial $T_{d}$ is defined
...
we have
\[
\P(\left|\sum^{s}_{i=1}Y_{i}\right|\geq a)\leq2\exp(-\frac{a^{2}}{2s}).
\]
\end{thm}

Now we are ready to show that there is a degree $\tilde{O}(\sqrt{s})$
polynomial that approximates $x^{s}$.
\begin{thm}
\label{thm:approximate_mono}For any positive integers $s$ and $d$,
there is a polynomial $p$ of degree at most $d$ such that
\[
...
\begin{proof}
Let $Y_{i}$ be i.i.d. random variables uniform on $\{-1,1\}$. Let
$Z_{s}=\sum^{s}_{i=1}Y_{i}$. Note that
\[
...
for all $x\in[-1,1]$ on the third line and the Chernoff bound (Theorem
...
\begin{thm}
For any $0<\mu\leq L$ and any $0<\epsilon\leq1$, there is a polynomial
$q$ of degree $O(\sqrt{\frac{L}{\mu}}\log(\frac{1}{\epsilon}))$
such that 
\[
q(0)=1\quad\text{and}\quad\max_{\mu\leq x\leq L}\left|q(x)\right|\leq\epsilon.
\]
\end{thm}
...
Since $|q(0)-1|\leq\epsilon/3\leq1/3$, rescaling by $1/q(0)$ changes
the bound on $[\mu,L]$ by only a constant factor, which can be absorbed
by adjusting the constants above. After this rescaling, $q$ satisfies
$q(0)=1$.\selectlanguage{english}%
\end{proof}
