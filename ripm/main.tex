...
\[
c^{\top}x=b^{\top}y+x^{\top}s\leq\max_{\ma^{\top}y+s=c,s\in\R^{n}_{\geq0}}b^{\top}y+x^{\top}s\leq\min_{x\in\PriR}c^{\top}x+x^{\top}s.
\]
\end{proof}
...
\]
Hence, we have $\|x's'-t'\|_{2}\leq\|\delta_{\mu}/\mu\|^{2}_{\mu}\leq0.15t\leq t'/6$.
...
For the second claim, wsing that $\nabla\Phi(r)=\sum^{n}_{i=1}\lambda\sinh(\lambda r_{i})$,
we have
\begin{align*}
...
Set $k\leftarrow0$, $x^{(0)}=\ox^{(0)}=x$, $s^{(0)}=\os^{(0)}=s$,
$r^{(0)}=\overline{r}^{(0)}=\frac{xs-t}{t}$.
...
$r^{(k+1)}\leftarrow(x^{(k+1)}s^{(k+1)}-t)/t$.
...
The intermediate variables $x^{(k)},s^{(k)},\ox^{(k)},\os^{(k)}$
can be stored implicitly and the cost of $\mathtt{SelectVector}$
roughly depends on how many coordinates of $x,s$ are updated. Hence,
the bottleneck of $\mathtt{FastRobustStep}$ solving the linear systems
\ref{eq:LP_newton_step_robust_fast}. The cost of solving a sequence
of linear systems in general depends on the number of entries changed.
Combining Lemmas we proved in the previous two subsections, we can
prove the correctness of $\mathtt{FastRobustStep}$ and bound the
...
Using $O(\sqrt{n}\log(t_{\mathrm{start}}/t_{\mathrm{end}}))$, $\mathtt{FastRobustStep}$
outputs $(x,s)$ such that
\[
...
\begin{proof}
Note that $\mathtt{FastRobustStep}$ is a instantiation of $\mathtt{RobustStep}$,
its output is analyzed in Lemma \ref{lem:RobustStep}. The sparsity
guarantee of $x$ and $s$ follows from Lemma \ref{lem:SelectVector}
with $\beta=\frac{1}{8\lambda}$ (Lemma \ref{lem:change_xsr}) and
$\delta\geq\frac{1}{48\lambda}$.
\end{proof}
...
\]
Note that we need change at least $n$ entries to get an approximate
...
system (Newton step) efficiently. There are many different ways on
...
changes during the algorithm, we can simplify by moving the vector
inside the matrix.
\begin{fact}
