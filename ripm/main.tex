...
\]
One key property about the primal and dual linear program is that
dual objective $b^{\top}y$ is a lower bound of the primal object
$c^{\top}y$ and that $x^{\top}s$ measure the gap between two objective.
...
\[
\max_{\ma^{\top}\oy+\os=c,\os\in\R^{n}_{\geq0}}b^{\top}\oy-x^{\top}s\leq b^{\top}y\leq c^{\top}x\leq\min_{\ma x=b,x\in\R^{n}_{\geq0}}c^{\top}\ox+x^{\top}s.
\]
\end{lem}
...
\[
\min_{\ma x=b,x\in\R^{n}_{\geq0}}c^{\top}\ox\geq\max_{\ma^{\top}y+s=c,s\in\R^{n}_{\geq0}}b^{\top}y.
\]
Using this, we have
\[
c^{\top}x=b^{\top}y+x^{\top}s\leq\max_{\ma^{\top}y+s=c,s\in\R^{n}_{\geq0}}b^{\top}y+x^{\top}s\leq\min_{\ma x=b,x\in\R^{n}_{\geq0}}c^{\top}\ox+x^{\top}s.
\]
The another direction is similar.
\end{proof}
...
$\delta_{\mu}/\mu$. For two vectors $u,v$, we define $\norm u_{v}\defeq\sqrt{u^{\top}\Diag(v)u}$
...
step size $h$. To ensure the feasibility, we need $\|\delta_{\mu}/\mu\|^{2}_{\mu}\leq\min_{i}\mu_{i}$
...
conclusion that the distance is less than $t/6$ is needed in Section
\ref{sec:IPM_interior}, where we call $\mathtt{L2Step}$ on a modified
LP and then prove that the result is close to the central path for
the original LP.
\begin{lem}
...
Using this, we have
\[
\|x's'-t'\|_{2}\leq\|\mu^{1/2}\mx^{-1}\delta_{x}\|_{2}\|\mu^{1/2}\ms^{-1}\delta_{s}\|_{2}=\|\mx^{-1}\delta_{x}\|_{\mu}\|\ms^{-1}\delta_{s}\|_{\mu}\leq\|\delta_{\mu}/\mu\|^{2}_{\mu}.
\]
...
we have
\[
\|\delta_{\mu}/\mu\|_{\mu}\leq\frac{t'}{t}\sqrt{\frac{4}{3t}}\|xs-t\|_{2}+h\sqrt{\frac{5}{4}tn}\leq\sqrt{\frac{t}{12}}+h\sqrt{\frac{5}{4}tn}\leq0.36\sqrt{t}.
\]
Hence, we have $\|x's'-t'\|_{2}\leq\|\delta_{\mu}/\mu\|^{2}_{\mu}\leq0.13t\leq t'/6$.
...
gives a good starting point of the original program. For technical
...
and any $0\leq\eta\leq\frac{1}{3}$. Consider the modified linear
program by
\[
...
\]
where $\overline{R}=6R/\eta$, $\overline{t}=4000n^{2}R/(\eta^{2}r)\cdot LR$,
$\ox^{+}=\overline{t}/(c+\overline{t}/\overline{R})$, $\ox^{-}=\ox^{+}-\ma^{\top}(\ma\ma^{\top})^{-1}b$,
$c^{+}=c$, $c^{-}=t/\ox^{-}$ and $\widetilde{b}=\sum^{n}_{i=1}\ox^{+}_{i}+\overline{R}$.
Then, we have that
\begin{enumerate}
...
Let $\epsilon=1/(48\sqrt{n})$, $\overline{R}=6R/\eta$, $\overline{t}=4000n^{2}R/(\eta^{2}r)\cdot LR$. 
...
$c^{-}$ and $\widetilde{b}$ are defined in Definition \ref{thm:modified_LP_construction}.
...
has inner radius $r$, outer radius $R$ and Lipschitz constant $L$(see
Definition \ref{def:LP_para}). Then, $\mathtt{SlowSolveLP}$ outputs
$x$ such that
\begin{align*}
...
For the last conclusion, we assume $\delta\leq\zeta$ and let $\mathcal{P}(\zeta)=\mathcal{P}\cap\{c^{\top}x\leq c^{\top}x^{*}+tLR\}$.
Note that $\mathcal{P}(\zeta)$ is a cone at $x^{*}$ (because there
is no vertex other than $x^{*}$ with value less than $c^{\top}x^{*}+\zeta LR$).
Hence, we have $\mathcal{P}(\delta)-x^{*}=\frac{\delta}{\eta}(\mathcal{P}(\zeta)-x^{*})$.
...
\end{align}
In each step, $x$, $s$ and $\delta_{\mu}$ in the equation above
change relatively by vectors with bounded $\ell_{2}$ norm. Thus,
only a few coordinates change substantially in each step. To take
advantage of this, the robust interior point method contains two new
components: (1) an analysis of convergence when we only solve the
linear system approximately (Section \ref{subsec:IPM_cosh_step}),
and (2) a method for maintaining the solution throughout the iteration
(Section \ref{subsec:select_xsr} and Section \ref{subsec:IPM_Inverse_Maintenance}).

\subsection{Staying within small $\ell_{\infty}$ distance\label{subsec:IPM_cosh_step}}

In the preceding description and analysis, we assumed that we computed
each step of the interior point method precisely. However, it may
suffice to compute steps approximately since our goal is only to stay
close to the central path. This could have significant computational
advantages.
...
$\|xs-t\|^{q}_{q}$. However, analyzing the step $\delta_{\mu}$ that
minimizes $\|\mu+\delta_{\mu}-t\|^{q}_{q}$ involves many cases. Instead,
we use the potential
\begin{equation}
\Phi(r)=\sum^{n}_{i=1}\cosh(\lambda r_{i})=\sum^{n}_{i=1}\frac{(e^{\lambda r_{i}}+e^{-\lambda r_{i}})}{2}.\label{eq:Phi_potential}
\end{equation}
with $r=\frac{xs-t}{t}$ for some scalar $\lambda=\Theta(\log n)$.
This potential induces the following algorithm, whose steps use $\delta_{\mu}\approx-c\nabla\Phi(\frac{xs-t}{t})$.
...
\textbf{Define} $\Phi(r)$ according to (\ref{eq:Phi_potential})
with $\lambda=16\log(40n)$.

\textbf{Invariant:} $(x,s)\in\PriR^{\circ}\times\DualR^{\circ}$ and
$\Phi((xs-t)/t)\leq16n$.

Let $t=t_{\mathrm{start}}$, $h=1/(128\lambda\sqrt{n})$, and let
$n$ be the number of columns in $\ma$.

\Repeat{$t=t_{\mathrm{end}}$}{

Pick $\ox$, $\os$ and $\overline{r}$ such that $\|\ln\ox-\ln x\|_{\infty}\leq\frac{1}{48}$,
$\|\ln\os-\ln s\|_{\infty}\leq\frac{1}{48}$ and $\|\overline{r}-\frac{xs-t}{t}\|_{\infty}\leq\frac{1}{48\lambda}$.
...
For any $r\in\Rn$, we have that $\|r\|_{\infty}\leq\frac{\log(2\Phi(r))}{\lambda}$
...
\begin{proof}
For every $i$, we have $\Phi(r)\geq\frac{1}{2}e^{\lambda|r_{i}|}$,
and hence $\|r\|_{\infty}\leq\frac{\log(2\Phi(r))}{\lambda}$.

For the second claim, using that $\nabla\Phi(r)=(\lambda\sinh(\lambda r_{i}))^{n}_{i=1}$,
we have
\begin{align*}
\|\nabla\Phi(r)\|_{2} & =\lambda\sqrt{\sum^{n}_{i=1}\sinh^{2}(\lambda r_{i})}=\lambda\sqrt{\sum^{n}_{i=1}(\cosh^{2}(\lambda r_{i})-1)}\\
 & \geq\frac{\lambda}{\sqrt{n}}\sum^{n}_{i=1}\sqrt{\cosh^{2}(\lambda r_{i})-1}\geq\frac{\lambda}{\sqrt{n}}\sum^{n}_{i=1}(\cosh(\lambda r_{i})-1)\\
 & =\frac{\lambda}{\sqrt{n}}(\Phi(r)-n).
\end{align*}

For the last claim, using $\sinh(r+\delta)=\sinh r\cosh\delta+\cosh r\sinh\delta$
and $\cosh r\leq|\sinh r|+1$, for $|\delta|\leq\frac{1}{5}$, we
have
\begin{align*}
...
By Lemma \ref{lem:Newton_step}, we have $\omx^{-1}\delta_{x}=(\mi-\mproj)(\overline{\delta}_{\mu}/\overline{\mu})$
where $\overline{\mu}=\ox\os$ and $\mproj=\oms^{-1}\ma^{\top}(\ma\oms^{-1}\omx\ma^{\top})^{-1}\ma\omx$.
By Lemma \ref{lem:Projection_property}, we have 
\[
\|\delta_{x}/\ox\|_{\overline{\mu}}=\|(\mi-\mproj)(\overline{\delta}_{\mu}/\overline{\mu})\|_{\overline{\mu}}\leq\|\overline{\delta}_{\mu}/\overline{\mu}\|_{\overline{\mu}}.
\]
...
and hence
\[
\|\delta_{x}/x\|_{2}\leq e^{1/48}\sqrt{\frac{11}{10t}}\|\delta_{x}/\ox\|_{\overline{\mu}}\leq e^{1/48}\sqrt{\frac{11}{10t}}\|\overline{\delta}_{\mu}\|_{\overline{\mu}^{-1}}\leq e^{1/48}\frac{11}{10t}\|\overline{\delta}_{\mu}\|_{2}
\]
Using the formula $\overline{\delta}_{\mu}=-\frac{t'}{32\lambda}\frac{\overline{g}}{\|\overline{g}\|_{2}}$,
we have
\begin{align*}
\|\delta_{x}/x\|_{2} & \leq e^{1/48}\frac{11}{10}\frac{t'}{32\lambda t}\leq\frac{1}{16\lambda}.
\end{align*}
The same proof gives $\|\delta_{s}/s\|_{2}\leq\frac{1}{16\lambda}$.
\end{proof}
...
\leq & \frac{1}{128\lambda}\frac{17}{16}+\frac{9}{8}(e^{1/48}-1)(\frac{1}{16\lambda}+\frac{1}{16\lambda})+\frac{9}{8}(\frac{1}{16\lambda}\frac{1}{16\lambda})\leq\frac{1}{80\lambda}.\label{eq:robust_IPM_eta}
\end{align}
...
we have
\[
\Phi(r')-\Phi(r)=\left\langle \overline{g}+\eta_{2},-\frac{1}{32\lambda}\frac{\overline{g}}{\|\overline{g}\|_{2}}+\eta\right\rangle =-\frac{1}{32\lambda}\|\overline{g}\|_{2}-\frac{1}{32\lambda}\eta^{\top}_{2}\frac{\overline{g}}{\|\overline{g}\|_{2}}+\overline{g}^{\top}\eta+\eta^{\top}_{2}\eta.
\]
Using $\|\eta_{2}\|_{2}\leq\frac{1}{3}\|\overline{g}\|_{2}$ and $\|\eta\|_{2}\leq\frac{1}{80\lambda}$
(\ref{eq:robust_IPM_eta}), we have
\begin{align*}
\Phi(r')-\Phi(r) & \leq-\frac{1}{32\lambda}\|\overline{g}\|_{2}+\frac{1}{32\lambda}\cdot\frac{1}{3}\|\overline{g}\|_{2}+\|\overline{g}\|_{2}\cdot\frac{1}{80\lambda}+\frac{1}{3}\|\overline{g}\|_{2}\cdot\frac{1}{80\lambda}\leq-\frac{1}{240\lambda}\|\overline{g}\|_{2}
\end{align*}
Using Lemma \ref{lem:robust_IPM_Phi}, we have $\|\overline{g}\|_{2}\geq\frac{\lambda}{\sqrt{n}}(\Phi(\overline{r})-n)\geq3\lambda\sqrt{n}$.
Hence, we have
\begin{equation}
\Phi(r')\leq\Phi(r)-\frac{\sqrt{n}}{80}<16n.\label{eq:IPM_robust_decrease}
\end{equation}
...
\begin{xca}
Show that we can use the potential function $\Phi=\sum^{n}_{i=1}\exp(\lambda|v_{i}|)$
...
shows that (recall $\lambda=16\log(40n)$):
\[
...
\]
Using this on (\ref{eq:robust_x_step}) gives $\|(x^{(k+1)}-x^{(k)})/x^{(k)}\|_{2}\leq\frac{8}{7}\frac{1}{t^{(k)}}\|\overline{\delta}_{\mu}\|_{2}.$Using
$\overline{\delta}_{\mu}=-\frac{t^{(k+1)}}{32\lambda}\frac{\overline{g}}{\|\overline{g}\|_{2}}$,
we have
\[
\|(x^{(k+1)}-x^{(k)})/x^{(k)}\|_{2}\leq\frac{1}{28\lambda}.
\]
To translate the bound to $\log$ scale, we note that $|\ln(1+t)|\leq2|t|$
...
such that $\|\overline{v}-v\|_{\infty}\leq\delta$. For every $2^{\ell}$
...
this step and $2^{\ell}$ steps earlier.
...
$I=I\cup\{i:|v^{(k)}_{i}-v^{(k-2^{\ell})}_{i}|\geq\delta/(2\left\lceil \log n\right\rceil )\}$.
...
$\overline{v}_{i}\leftarrow v^{(k)}_{i}$ for all $i\in I$
...
\item $\|\overline{v}^{(k)}-\overline{v}^{(k-1)}\|_{0}\leq O(\min(2^{2\ell_{k}}(\beta/\delta)^{2}\log^{2}n,n))$
where $\ell_{k}$ is the largest integer $\ell$ with $k=0\mod 2^{\ell}$. 
\end{itemize}
\end{lem}
...
namely, $\overline{v}^{(k)}_{i}=\overline{v}^{(k')}_{i}=v^{(k')}_{i}$.
...
Hence, we have that
\[
v^{(k)}_{i}-\overline{v}^{(k)}_{i}=v^{(k_{s})}_{i}-v^{(k_{0})}_{i}=\sum^{s-1}_{j=0}(v^{(k_{j+1})}_{i}-v^{(k_{j})}_{i}).
\]
Since $\overline{v}_{i}$ is not updated since step $k'$, we have
$|v^{(k_{j+1})}_{i}-v^{(k_{j})}_{i}|\leq\delta/(2\left\lceil \log n\right\rceil )$
and hence $|v^{(k)}_{i}-\overline{v}^{(k)}_{i}|\leq\delta$. Since
this holds for every $i$, we have that $\|\overline{v}^{(k)}-v^{(k)}\|_{\infty}\leq\delta$.

For the sparsity of $\overline{v}^{(k+1)}-\overline{v}^{(k)}$, we
first bound the size of the set $I_{\ell}\defeq\{i:|v^{(k)}_{i}-v^{(k-2^{\ell})}_{i}|\geq\delta/(2\left\lceil \log n\right\rceil )\}$.
Note that
\[
|I_{\ell}|\cdot\frac{\delta^{2}}{5\log^{2}n}\leq\sum^{n}_{i=1}|v^{(k)}_{i}-v^{(k-2^{\ell})}_{i}|^{2}\leq2^{\ell}\sum^{n}_{i=1}\sum^{k-1}_{t=k-2^{\ell}}|v^{(t+1)}_{i}-v^{(t)}_{i}|^{2}\leq2^{2\ell}\beta^{2}
\]
...
\[
|I|\leq\sum^{\ell_{k}}_{\ell=0}|I_{\ell}|=O(2^{2\ell_{k}}(\beta/\delta)^{2}\log^{2}n).
\]
\end{proof}
...
$\mathtt{RobustStep}$ and $\mathtt{SelectVector}$. The algorithm
is given in Algorithm \ref{alg:FastRobustStep}.
...
Let $h=1/(128\lambda\sqrt{n})$, and let $n$ be the number of columns
in $\ma$.

Set $k\leftarrow0$, $t^{(0)}=t_{\mathrm{start}}$, $x^{(0)}=\ox^{(0)}=x$,
$s^{(0)}=\os^{(0)}=s$, $r^{(0)}=\overline{r}^{(0)}=\frac{xs-t^{(0)}}{t^{(0)}}$.

\Repeat{$t^{(k)}=t_{\mathrm{end}}$}{

Let $t^{(k+1)}=\max(t^{(k)}/(1+h),t_{\mathrm{end}})$, $\overline{\delta}_{\mu}=-\frac{t^{(k+1)}}{32\lambda}\frac{\overline{g}}{\|\overline{g}\|_{2}}$,
...
Set $x^{(k+1)}\leftarrow x^{(k)}+\delta_{x}$, $s^{(k+1)}\leftarrow s^{(k)}+\delta_{s}$,
$r^{(k+1)}\leftarrow(x^{(k+1)}s^{(k+1)}-t^{(k+1)})/t^{(k+1)}$.
...
\textbf{Return} $(x^{(k)},s^{(k)})$.

\end{algorithm2e}

The intermediate variables $x^{(k)},s^{(k)},\ox^{(k)},\os^{(k)},\overline{r}^{(k)}$
can be stored implicitly and the cost of $\mathtt{SelectVector}$
roughly depends on how many coordinates of $\ox,\os,\overline{r}$
are updated. Hence, the bottleneck of $\mathtt{FastRobustStep}$ is
solving the linear systems in \ref{eq:LP_newton_step_robust_fast}.
The cost of solving a sequence of linear systems in general depends
on the number of entries changed. Combining the lemmas proved in the
previous two subsections, we can prove the correctness of $\mathtt{FastRobustStep}$
and bound the number of entries changed.
\begin{lem}
\label{lem:FastRobustStepMeta}Suppose that the input satisfies $(x,s)\in\PriR^{\circ}\times\DualR^{\circ}$
and $\Phi_{\mathrm{start}}\defeq\Phi((xs-t_{\mathrm{start}})/t_{\mathrm{start}})\leq16n$.
After $O(\sqrt{n}\log(t_{\mathrm{start}}/t_{\mathrm{end}}))$ iterations,
$\mathtt{FastRobustStep}$ outputs $(x,s)$ such that
\[
...
\begin{proof}
Note that $\mathtt{FastRobustStep}$ is an instantiation of $\mathtt{RobustStep}$,
and its output is analyzed in Lemma \ref{lem:RobustStep}. The sparsity
guarantees for $\overline{x}$, $\overline{s}$, and $\overline{r}$
follow from Lemma \ref{lem:SelectVector} with $\beta=\frac{1}{8\lambda}$
(Lemma \ref{lem:change_xsr}) and $\delta\geq\frac{1}{48\lambda}$.
\end{proof}

As we see from Section \ref{subsec:SolveLP_apx}, we can solve a linear
program by calling $\mathtt{FastRobustStep}$ (or $\mathtt{L2Step}$)
twice and that 
\[
...
\[
\sum^{T}_{k=1}O(\min(2^{2\ell_{k}}\log^{2}n,n))=O(n\log n\cdot\log(\frac{nR}{\delta r})).
\]
Note that we need to change at least $n$ entries to get an approximate
solution. Hence, the number of changes is optimal up to a polylogarithmic
factor. 
...
system (Newton step) efficiently. There are many different ways of
maintaining the solution depending on the structure of $\ma$. In
this note, we consider the generic case that $\ma$ is an arbitrary
matrix. Although both the matrix and the vector of the Newton step
change during the algorithm, we can simplify by moving the vector
into the matrix.
\begin{fact}
...
with $q$ coordinates modified in total, there are only $q$ entries
...
\begin{lem}
\label{lem:maintain_inverse}Given vectors $\ox,\os,\overline{r}\in\R^{n}$and
an update $\delta_{\ox},\delta_{\os},\delta_{\overline{r}}\in\R^{n}$,
...
\begin{itemize}
\item Given $\mm^{-1}_{\ox,\os,\overline{r}}$, we can compute $\mm^{-1}_{\ox+\delta_{\ox},\os+\delta_{\os},\overline{r}+\delta_{\overline{r}}}$
in time $O(T_{n,q,n})$.
\item Given $\mm^{-1}_{\ox,\os,\overline{r}}$ and $\mm^{-1}_{\ox,\os,\overline{r}}b$,
we can compute $\mm^{-1}_{\ox+\delta_{\ox},\os+\delta_{\os},\overline{r}+\delta_{\overline{r}}}b$
...
\]
where $\mathbf{U}$ consists of $q$ columns of the identity matrix,
$\mathbf{C}$ is a $q\times q$ matrix and $\mathbf{V}$ consists
of $q$ rows of the identity matrix. Hence, the Woodbury matrix identity
shows that
\begin{equation}
\mm^{-1}_{1}=(\mm_{0}+\mathbf{U}\mathbf{C}\mathbf{V})^{-1}=\mm^{-1}_{0}-\mm^{-1}_{0}\mathbf{U}(\mathbf{C}^{-1}+\mathbf{V}\mm^{-1}_{0}\mathbf{U})^{-1}\mathbf{V}\mm^{-1}_{0}.\label{eq:Woodbury}
\end{equation}
Note that $\mm^{-1}_{0}\mathbf{U}$, $\mathbf{V}\mm^{-1}_{0}\mathbf{U}$,
$\mathbf{V}\mm^{-1}_{0}$ are just blocks of $\mm^{-1}_{0}$ and no
computation is needed. Hence, we can compute $(\mathbf{C}^{-1}+\mathbf{V}\mm^{-1}_{0}\mathbf{U})^{-1}$
...
For computing $\mm^{-1}_{1}b$, we note that
\[
\mm^{-1}_{1}b=\mm^{-1}_{0}b-\mm^{-1}_{0}\mathbf{U}(\mathbf{C}^{-1}+\mathbf{V}\mm^{-1}_{0}\mathbf{U})^{-1}\mathbf{V}\mm^{-1}_{0}b.
\]
Since $\mm^{-1}_{0}b$ is given, the above formula can be computed
...
\caption{$\mathtt{InverseMaintenance}()$}

Set $k\leftarrow0$ and $\ell_{*}$ such that $2^{2\ell_{*}}=\Theta(\min(n^{\alpha},n^{2/3}))$.

\While{Not finished}{

\textbf{Obtain} the current $\ox^{(k)},\os^{(k)},\overline{r}^{(k)}$.

\uIf{$k=0\mod 2^{\ell_{*}}$}{

Update $\mathbf{T}$ to $\mm^{-1}_{\ox^{(k)},\os^{(k)},\overline{r}^{(k)}}$
...
Update $v$ to $\mm^{-1}_{\ox^{(k)},\os^{(k)},\overline{r}^{(k)}}e_{2n+d+1}$
...
Combining Lemma \ref{lem:maintain_inverse} with Lemma \ref{lem:SelectVector},
we have the following guarantee.
\begin{lem}
\label{lem:FastRobustStepTime}Using $\mathtt{InverseMaintenance}$
to implement Line \ref{eq:LP_newton_step_robust_fast} of $\mathtt{FastRobustStep}$
(Algorithm \ref{alg:FastRobustStep}), $\mathtt{FastRobustStep}$
takes in total time 
\[
O((n^{\omega+o(1)}+n^{2+1/6+o(1)}+n^{5/2-\alpha/2+o(1)})\log(t_{\mathrm{start}}/t_{\mathrm{end}})).
\]
\end{lem}
...
is the largest integer $\ell$ with $k=0\mod 2^{\ell}$. We can now
bound all the computation costs as follows.

\textbf{Cost of updating $v$}: We update $u$ whenever $k=0\mod 2^{\ell_{*}}$.
Within that $2^{\ell_{*}}$ steps, the number of coordinates updated
in $\ox,\os,\overline{r}$ is bounded by
\[
q\defeq\sum^{2^{\ell_{*}}-1}_{k=1}q_{k}=\sum^{2^{\ell_{*}}-1}_{k=1}O(2^{2\ell_{k}}\log^{2}n)=O(2^{2\ell_{*}}\log^{2}n).
\]
...
\]
time to compute $v=\mm^{-1}_{\ox^{(k)},\os^{(k)},\overline{r}^{(k)}}e_{2n+d+1}$
...
\begin{align*}
\widetilde{\widetilde{O}}(\sum^{\frac{1}{2}\log n}_{\ell=\ell_{*}}2^{-\ell}T_{n,n,2^{2\ell}}) & =\widetilde{\widetilde{O}}(\sum^{\frac{1}{2}\log n}_{\ell=\ell_{*}}(n^{\omega-\frac{\omega-2}{1-\alpha}}2^{2\ell\cdot\frac{\omega-2}{1-\alpha}-\ell}+n^{2}2^{-\ell}))\\
 & =\widetilde{\widetilde{O}}(\sum^{\frac{1}{2}\log n}_{\ell=\ell_{*}}n^{\omega-\frac{\omega-2}{1-\alpha}}2^{2\ell\cdot\frac{\omega-2}{1-\alpha}-\ell}+n^{2}2^{-\ell_{*}})
\end{align*}
...
Since there are $\sqrt{n}\log(t_{\mathrm{start}}/t_{\mathrm{end}})$
steps up to constants, the total cost is
\begin{align*}
 & \widetilde{\widetilde{O}}(n^{\omega}+\sqrt{n}\log(t_{\mathrm{start}}/t_{\mathrm{end}})(2^{2\ell_{*}\omega}+n2^{2\ell_{*}}+n^{\omega-\frac{1}{2}}+n^{2}2^{-\ell_{*}}))\\
= & \widetilde{\widetilde{O}}(\sqrt{n}\log(t_{\mathrm{start}}/t_{\mathrm{end}})(n2^{2\ell_{*}}+n^{\omega-\frac{1}{2}}+n^{2}2^{-\ell_{*}}))
\end{align*}
where we used $2^{2\ell_{*}\omega}\leq n^{\alpha\omega}\leq n$. Putting
$2^{2\ell_{*}}=\min(n^{\alpha},n^{2/3})$, we have
\[
\widetilde{\widetilde{O}}((n^{\omega}+n^{2+1/6}+n^{5/2-\alpha/2})\log(t_{\mathrm{start}}/t_{\mathrm{end}})).
\]
\end{proof}
...
(see Definition \ref{def:LP_para}). Then we can find $x$ such that
\begin{align*}
c^{\top}x & \leq\min_{\ma x=b,x\geq0}c^{\top}x+\delta LR,\\
\ma x & =b,\\
x & \geq0.
...
\begin{proof}
The algorithm for finding $x$ is the same as $\mathtt{SlowSolveLP}$
...
is an instantiation of $\mathtt{RobustStep}$, its output is analyzed
...
First, we show that $(\ox^{+},\ox^{-},\overline{R})$ defined in Theorem
\ref{thm:modified_LP_construction} is indeed on the central path
of the modified linear program. First, we define the dual of the modified
...
Using $\ma\ma^{\top}(\ma\ma^{\top})^{-1}b=b$, one can check it satisfies
all the equality constraints above. 
...
is the least square solution to $\ma x=b$, so, we have 
\begin{equation}
...
$(\ox^{+},\ox^{-},\overline{R})$ is on the central path at $t$.
\end{proof}
...
\]
Similarly, we have $\widetilde{b}=\sum_{i}x^{+}_{c,i}+\overline{R}\geq\frac{4}{5}n\overline{R}$. 

For the bound on $c^{-}_{i}$, recall that $c^{-}_{i}=t/\ox^{-}_{i}$
with $\ox^{-}=\ox^{+}-x_{\circ}$ and $x_{\circ}=\ma^{\top}(\ma\ma^{\top})^{-1}b$.
Using (\ref{eq:x0_upper_bound}), we have 
\[
x^{-}_{c,i}\leq\frac{4}{3}\overline{R}+R.
\]
...
vector is positive. Since $\ox^{-}>0$, we have $v^{-}>0$. Since
$v\geq r$, we also have $v^{+}>0$. For $v^{\theta}$, we use $\widetilde{b}\geq\frac{4}{5}n\overline{R}$
(Lemma \ref{lem:ipm_propert_modified}), $v\leq R$ and $v^{-}\leq\frac{4L\overline{R}}{t}\cdot R\leq R$
to get
\begin{equation}
v^{+}_{i}=v+v^{-}\leq2R\label{eq:v_plus_upper}
\end{equation}
and hence
\begin{align*}
...
and $\overline{t}$, Lemma \ref{lem:LP_exact_distance_OPT} shows
that 
\[
...
\[
\frac{s^{\theta}}{\min_{i}s^{+}_{i}}\leq\frac{\frac{3LR}{n\overline{R}}}{L/2}=\frac{6R}{n\overline{R}}<\eta
\]
...
is an explicit formula involving $(\ma(\mw_{1}+\mw_{2})\ma^{\top})^{-1}v_{1:d}$
and $(\ma(\mw_{1}+\mw_{2})\ma^{\top})^{-1}\ma w_{1}$. Hence, we can
compute $\mh^{-1}v$ by solving two linear systems of the form $\ma\mw\ma^{\top}$
