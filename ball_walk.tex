...
$1/2$, we do nothing; with the rest we take a step according to the
Markov chain. The next theorem is folklore.
\end{defn}
...
\]
Since the walk is lazy, $P_{u}(A)\geq\frac{1}{2}$ iff $u\in A$,
the range of the functions $g_{1},g_{2}$ is $[0,1]$. We let 
\[
...
be s.t. $Q(\{x:f(x)>r\})\le1/2$ and $Q(\{x:f(x)<r\})\le1/2$. Let
$g(x)=\max\{f(x)-r,0\}$. We then have
\[
...
As a result, we have 
\[
\langle Mf,Mf\rangle\le\left(1-\frac{\phi^{2}}{4}\right)^{2}.
\]
...
ball walk for convex body. However, when the starting distribution
...
In the section we bound the conductance of the ball walk when applied
...
is simpler, but less efficient is to ``smoothen'' the convex body
...
have a significant overlap if the points are sufficient close.
\begin{lem}[One-step overlap]
\label{lem:ONESTEP} Let $u,v\in K$ s.t. $\ell(u),\ell(v)\ge\ell$
...
\begin{figure}[H]
\centering{}\includegraphics[width=0.55\columnwidth]{Figures/Partitions}\caption{Partitions of convex body}
\end{figure}
...
\]
Thus $d(S_{1},S_{2})\geq\ell\delta/2\sqrt{n}$. Applying Theorem \ref{thm:isoD}
to the partition $S_{1}',S_{2}',S_{3}'$, we have 
\begin{eqnarray*}
...
to $K_{\delta},$the points in $K$ that have high local conductance.
...
Input: membership oracle for $K$ s.t. $B^{n}\subseteq K\subseteq RB^{n}$.
...
\begin{thm}
For an isotropic logconcave distribution $Q$ in $\R^{n}$, the covariance
matrix from $N=O(n)$ random samples satisfies $\norm{A-I}_{\op}\le0.5$.
\end{thm}
...
boundary, diameter $D$ and $Ric_{g}\geq0$, then the Poincaré constant
...
has been extended to Riemannian setting \cite{klartag2017needle}.
...
\begin{proof}
WLOG we can assume that the functions $g,h$ are continuous. This
is because they are limits of monotone increasing sequences of continuous
functions $g_{k},h_{k}$ where 
\[
...
(i) minimal support $\norm{a-b}$; WLOG assume $a=0,b=e_{1}$, and
(ii) is linear in the segment $[\alpha,\beta]$ for $0\le\alpha\le\beta\le1$
with $\beta-\alpha$ is maximal. It will be useful to view $\psi$
as defining a convex body $K'$ whose slices are simplices:
\[
...
\]
and similarly for $\int_{\K'}\hat{h}\ge0$. WLOG we can assume that
$\int_{\K'}\hat{g}=0$.

\begin{figure}[H]
\centering{}\includegraphics[width=0.5\textwidth]{Figures/concave-to-affine}\caption{Going to an affine needle}
\end{figure}
...
applied to the function $F(t)$ contradicts (\ref{FEQ}) and to prove
the theorem in general, and it suffices to prove it in the one-dimensional
...
proved by Kannan-Lovász-Simonovits in the same paper in which they
...
Here we give some alternative equivalent version of Lemma \ref{lem:LOCALIZATION}
...
\begin{lem}[Localization: Product form]
 Let $f_{1},f_{2},f_{3},f_{4}:[a,b]\rightarrow\R_{+}$ be nonnegative,
...
\]
iff for every subinterval $\ensuremath{[a',b']\subseteq[a,b]}$, and
every $\gamma\in\R$, 
\[
...
\item the distributions $v$ satisfies
\begin{enumerate}
\item density function is of the form $e^{\ell}$ with linear $\ell$,
\item support equals to a segment $[a,b]\subset K$,
\item $\int fdv=0$,
