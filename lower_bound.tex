...
\begin{proof}
First, we check the strong convexity and smoothness. Note that $f(x)=-\frac{L-\mu}{4}x_{1}+\frac{1}{2}x^{\top}Ax$
...
is supported only on the first coordinate. Hence, by the assumption,
$x^{(1)}$ is supported only on the first coordinate and $\nabla f(x^{(1)})$
is supported only on the first two coordinates. By induction, only
the first $k$ coordinates of $x^{(k)}$ are non-zero.
...
and that
\[
\|x^{(0)}-x^{*}\|^{2}_{2}\leq\sum^{\infty}_{i=1}q^{2i}=\frac{q^{2}}{1-q^{2}}=q^{2}\frac{(\sqrt{L/\mu}+1)^{2}}{4\sqrt{L/\mu}}\leq q^{2}\frac{\sqrt{L/\mu}+1}{2},\quad q=\frac{\sqrt{L/\mu}-1}{\sqrt{L/\mu}+1}.
\]
...
Thus, it is a problem we need to address. In some sense, the proof
points out a common issue of any algorithm that only uses gradient
information. Given any convex function, we construct the dependence
graph $G$ on the set of variables $x_{i}$ by connecting $x_{i}$
to $x_{j}$ if $\nabla f(x)_{i}$ depends on $x_{j}$ or $\nabla f(x)_{j}$
depends on $x_{i}$ (given all other variables). Note that the dependence
