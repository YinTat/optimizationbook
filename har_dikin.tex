...
\[
\left|\psi'(t)\right|=\left|v^{\top}\frac{d}{dt}\mh(x_{t})v\right|\leq2\|h\|_{x_{t}}\|v\|^{2}_{x_{t}}=2\phi(t)\psi(t).
\]
...
\begin{center}
\includegraphics[scale=0.45]{Figures/symmetry.png}
\par\end{center}%
\end{minipage}\hfill{}%
\begin{minipage}[t]{0.45\textwidth}%
\noindent\begin{flushleft}
\includegraphics[scale=0.35]{Figures/ssc.png}
\par\end{flushleft}%
\end{minipage}
...
\item $\Pr_{w\sim E_{x}(r)}\left(\norm{w-x}_{x}\le r^{2}\left(1-\frac{1}{n}\right)\right)\ge1-c_{0}r$
for some constant $c_{0}$.
\item $\Pr_{w\sim E_{x}(r)}\left(\norm{w-x}_{w}+\norm{w-x}_{2x-w}\le2r^{2}\left(1-\frac{1}{n}\right)\vert\norm{w-x}_{x}\le r^{2}\left(1-\frac{1}{n}\right)\right)\ge1-c_{1}r$
...
For the rejection probability at $x$, we consider the algorithm pick
$z$ from $E_{x}(r)$. Let $f(z)=\ln\det\mathbf{H}(z)$. The acceptance
...
\end{equation}
To compute $\|\nabla f(x)\|^{2}_{2}$, it is easier to compute directional
derivative of $\nabla f$. Note that 
\begin{align}
...
at $x,y$. Again, we can assume that $\mh(x)=\mi$. Then, the strongly
self-concordance and Lemma \ref{lem:global_strongly_self_concordant}
shows that
\begin{equation}
...
Therefore, 
\[
(y-x)^{\top}\nabla^{2}\phi(x)(y0x)=\sum^{m}_{i=1}\frac{\left(a^{\top}_{i}(y-x)\right)^{2}}{\left(a^{\top}_{i}x-b_{i}\right)^{2}}\le m.
\]
\end{proof}
...
\mbox{using isoperimetry (Theorem \ref{thm:iso-cross-ratio})} & \ge\frac{1}{16}d_{K}(S_{1}',S_{2}')\frac{\vol(S_{1}')\vol(S_{2}')}{\vol(P)}\\
