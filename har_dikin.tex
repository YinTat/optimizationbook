...
\]
We will now apply Thm. \ref{thm:har_weighted_iso} using $h(x)=\frac{s(x)}{48\sqrt{n}D}$
...
the Hessian remains finite for every point in $[x,y]$ for $y$ s.t.
$\norm{x-y}_{x}<1$, we have that $\phi(y)$ is also finite, and hence
$y\in K$, since $\phi$ is a barrier function for $K$. 
