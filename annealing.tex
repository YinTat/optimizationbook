...
$f:\R^{n}\rightarrow\R_{+}$, a point $x_{0}$ such that $f(x_{0})\ge\beta,$
numbers $r,R>0$ such that (i) $\E(\norm{x-x_{0}}^{2})\le R^{2},$
(ii) the level set of $\pi_{f}$ of measure $1/8$ contains a ball
of radius $r$, and $\varepsilon>0$, output a number $A$ such that
\[
...
of oracle calls and the number of arithmetic operations. 
\end{problem}
...
\hline 
1990/Lovász-Simonovits \cite{LS90} & Better isoperimetry & $n^{16}$\tabularnewline
\hline 
1990/Lovász \cite{L90} & Ball walk & $n^{10}$\tabularnewline
\hline 
...
\hline 
1993/Lovász-Simonovits \cite{LS93} & Localization lemma & $n^{7}$\tabularnewline
\hline 
1997/Kannan-Lovász-Simonovits \cite{kannan1997random} & Speedy walk, isotropy & $n^{5}$\tabularnewline
\hline 
2003/Lovász-Vempala \cite{LV2003} & Annealing, hit-and-run & $n^{4}$\tabularnewline
\hline 
...
\hline 
\end{tabular}\caption{\label{tab:volume} Complexities for volume estimation; each step
uses $\widetilde{O}(n)$ bits of randomness. The last algorithm needs
...
$K_{0}=B^{n}\subset K_{1}\subseteq\ldots\subseteq K_{m}=K$, starting
...
\]
 The length of the sequence is $m=\lceil n\log_{2}R\rceil$ so that
the final body is just $K$. We can now state the algorithm. In each
iteration the algorithm estimates the ratio $\vol(K_{i})/\vol(K_{i+1})$
by sampling uniformly from $K_{i+1}$ and using the fraction of the
sample that lies in $K_{i}$ as the estimator.
...
\end{enumerate}
\item Let $Y=\prod^{m-1}_{i=0}Y_{i}$ and return $\vol(B^{n})/Y$.
\end{enumerate}
\end{algorithm2e}
...
We have 
\[
K_{i+1}=2^{(i+1)/n}B^{n}\cap K\subseteq2^{1/n}(2^{i/n}B^{n}\cap K)=2^{1/n}K_{i}.
\]
The lemma follows.
\end{proof}
...
us the ``right'' bound.
\begin{lem}
\label{lem:prod_var}Let $Y=\prod^{m-1}_{i=0}Y_{i}$ where $Y_{i}\ge0$
...
\begin{thm}
For any input convex body $K$ satisfying $B^{n}\subseteq K\subseteq RB^{n}$
...
\[
\frac{\Var(Y_{i})}{\E(Y_{i})^{2}}=\frac{r_{i}(1-r_{i})}{Nr^{2}_{i}}=\frac{1-r_{i}}{Nr_{i}}\le\frac{1}{N}
\]
...
and this means that $m=\Omega(n)$ is needed even to get polynomial
complexity. We will soon see that this intuitive reasoning is not
valid. 
...
\[
\int_{\R^{n}}f=\int f_{0}\frac{\int f_{1}}{\int f_{0}}\frac{\int f_{2}}{\int f_{1}}\cdots\frac{\int f_{m}}{\int f_{m-1}}
\]
where $f_{0}$ is chosen to be a function with an easy integral (typically
a formula) and $f_{m}=f$. To compute any ratio $\int g/\int h$ we
...
\begin{lem}
For integrable functions $g,h$ with support of $g$ contained in
the support of $h$ and $g(X)/h(X)$ defined everywhere in the support
of $h$, for $X$ drawn from the distribution with density proportional
to $h$ and $Y=g(X)/h(X)$, we have 
\[
...
\]
\item Let $X^{1}_{0},\ldots,X^{k}_{0}$ be independent random points with
density proportional to $f_{0}$.
\item For $i=0,\ldots,m-1$: starting with $X^{1}_{i},\ldots,X^{k}_{i}$,
generate random points $X_{i+1}=\{X^{1}_{i+1},\ldots,X^{k}_{i+1}\}$;
...
satisfies
\[
\E_{X\sim p_{i}}\left(\frac{p_{i}(X)}{p_{i+1}(X)}\right)=O(1).
\]
\end{lem}
...
Then $Z(a)=a^{n}\int_{\R^{n}}f(x)^{a}\,dx$ is logconcave for $a>0$.
If $f$ has support $K$, then $Z(a)=a^{n}\int_{K}f(x)^{a}\,dx$ is
logconcave for $a>0$.
\end{lem}
...
Now by the Prekopa-Leindler Theorem (see Lemma \ref{lem:marginal}),
the marginal of $F$ in $t$ is also logconcave. This marginal is
\[
...
$\mathrm{var}(Y)/\mathbb{E}[Y]^{2}$ of the random variable given
by $Y=f_{i+1}(X)/f_{i}(X)$ for a random sample $X$ drawn from the
distribution with density proportional to $f_{i}$.
