...
Newton-Raphson iteration approximates a function by its gradient/Jacobian,
then guesses where the gradient intersects the zero line as a likely
zero, repeating this process until a sufficient approximation has
been found. In one dimension, we approximate the function $g$ by
its gradient 
\[
...
\textbf{Input:} Initial point $x^{(0)}\in\Rn$
...
The result follows.
\end{proof}
This argument above uses only that $g\in\mathcal{C}^{2}$ and does
...
\begin{thm}
Given a polynomial $g(x)=\sum^{n}_{i=0}a_{i}x^{i}$ with only real
roots. Let $\lambda_{1}$ be its largest root. Suppose that $\lambda_{1}\leq x^{(0)}$.
...
\]
For any $x\geq\lambda_{1}$, we have 
\begin{align*}
...
enough to a root.
\begin{thm}
Assume that $|f'(x^{*})|\geq\alpha$ at $f(x^{*})=0$ and $f'$ is
...
Therefore, 
\begin{align*}
x^{(k+1)}-x^{*} & =\frac{1}{f'(x^{(k)})}\int^{x^{*}}_{x^{(k)}}(f'(z)-f'(x^{(k)}))dz.\\
 & \leq\frac{L}{|f'(x^{(k)})|}\int^{x^{*}}_{x^{(k)}}|z-x^{(k)}|dz\\
...
\]
implying $|x^{(k)}-x^{*}|<\epsilon$ after $\log\log(\frac{L\epsilon_{0}}{\alpha\epsilon})$
steps. Note that $f$ has not been assumed to be convex or polynomial.
\end{proof}
...
points close enough to the optimal.
...
using the function itself. For one dimension, we can approximate the
Newton method and get the following secant method 
\[
x^{(k+1)}=x^{(k)}-g(x^{(k)})\frac{x^{(k)}-x^{(k-1)}}{g(x^{(k)})-g(x^{(k-1)})}
\]
where we approximated the $g'(x^{(k)})$ by $\frac{g(x^{(k)})-g(x^{(k-1)})}{x^{(k)}-x^{(k-1)}}$.
...
Jacobian we maintained in the $k^{th}$ iteration. Similar to the
secant method, we want to enforce
\[
...
In practice, the recursive formula $J^{(k)}$ becomes too expensive
and hence one can stop the recursive formula after constant steps,
