...
Grötschel, Lov\'asz and Schrijver \cite{grotschel2012geometric}
...
\[
\E\zeta\leq2n^{3/2}\frac{r_{2}}{r_{1}}L+\frac{\varepsilon n}{r_{2}}.
\]
...
either asserts $x\in B(K,\varepsilon)$ or outputs a halfspace that
contains $K$. 
\end{lem}
...
\[
\text{SEP}_{O(\max\{n^{7/6}\kappa^{2}\delta^{1/3}/\rho,\rho,n\log(1/\delta)\delta\})}(K)\leq O(n\log(1/\delta))\text{MEM}_{\delta}(K).
\]
Setting $\rho=\sqrt{n^{7/6}\kappa^{2}\delta^{1/3}}$ and $\delta=\Theta\left(\frac{\eta^{6}}{n^{7/2}\kappa^{6}}\right)$,
we have that 
\[
\text{SEP}_{\eta}(K)\leq O(n\log(\frac{n\kappa}{\eta}))\text{MEM}_{\eta^{6}/(n^{7/2}\kappa^{6})}(K).
\]
\end{proof}
