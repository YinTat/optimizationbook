...
\min_{x\in\Rn}f(x)
\]
} & {\scriptsize$\nabla^{2}f\succeq\mu\cdot I,\partial^{2}_{i}f\leq L_{i}$} & {\scriptsize Accelerated Coordinate Descent} & \multicolumn{2}{c|}{{\scriptsize Total Cost: $\sum_{i}\sqrt{\beta_{i}/\mu}\mathcal{T}_{\partial_{i}f}\log(\frac{1}{\epsilon})$}}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
In the table, we ignored some entries inside the $\log$ which leads
to an ``incorrect'' unit. However, when everything is written correctly,
the number of iterations must be unitless. Furthermore, we can view
$GR$, $LR^{2}$, $\frac{G^{2}}{\mu}$ all as upper bounds of the
initial error $f(x^{(0)})-f^{*}$. For example, we know that
\[
