...
We have now seen various algorithms to minimize a convex function.
They have different runtimes and seem difficult to compare. Before
going deeper and discussing more results, here we discuss how to understand
...
\min_{x}f(x)
\]
} & \centering{}{\scriptsize -} & {\scriptsize Hybrid Center Cutting Plane} & {\scriptsize$n\log(\frac{n}{\epsilon})$} & {\scriptsize$\mathcal{T}_{f}+n^{2}$}\tabularnewline
\hline 
{\scriptsize
\[
\min_{x\in\Omega_{1}\cap\Omega_{2}}c^{\top}x
\]
} & \centering{}{\scriptsize -} & {\scriptsize Hybrid Center Cutting Plane} & {\scriptsize$n\log(\frac{n}{\epsilon})$} & {\scriptsize$\mathcal{T}_{\Omega_{1}}+\mathcal{T}_{\Omega_{2}}+n^{2}$}\tabularnewline
\hline 
{\scriptsize
\[
\min_{x}f(x)+g(x)
...
\]
} & {\scriptsize$\nabla^{2}f\succeq\mu\cdot I,\nabla^{2}f_{i}\preceq L_{i}\cdot I$} & {\scriptsize Accelerated Stochastic Descent} & \multicolumn{2}{c|}{{\scriptsize Total: $(\mathcal{T}_{\nabla f}+\sqrt{\frac{1}{\mu}\sum_{i}L_{i}}\mathcal{T}_{\nabla f_{i}})\log(\frac{1}{\epsilon})$}}\tabularnewline
\hline 
{\scriptsize
\[
\min_{x\in\Rn}f(x)
\]
} & {\scriptsize$\nabla^{2}f\succeq\mu\cdot I,\partial^{2}_{i}f\leq L_{i}$} & {\scriptsize Accelerated Coordinate Descent} & \multicolumn{2}{c|}{{\scriptsize Total: $\sum_{i}\sqrt{L_{i}/\mu}\mathcal{T}_{\partial_{i}f}\log(\frac{1}{\epsilon})$}}\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
\hline 
\multirow{2}{0.1\paperwidth}{\centering{}{\scriptsize$\min_{Ax=b,x\in\Rn_{+}}c^{\top}x$}} & \multirow{2}{0.15\paperwidth}{\centering{}{\scriptsize -}} & {\scriptsize Interior Point Method} & {\scriptsize$\sqrt{n}\log^{O(1)}(n)\log(\frac{1}{\epsilon})$} & {\scriptsize$n^{2.3728\cdots}\log(\frac{1}{\epsilon})$}\tabularnewline
\cline{3-5}
...
\centering{}{\scriptsize
\[
\min_{x}\sum^{n}_{i=1}f_{i}(a^{\top}_{i}x)
\]
...
with a rate of $\log(1/\epsilon)$. For all of these problems, the
...
of gradient descent $\left(\frac{GR}{\epsilon}\right)^{2}$ is unitless.
In the tables, we have ignored some entries inside the $\log$ which
leads to ``incorrect'' units. However, when everything is written
correctly, the number of iterations must be unitless. Furthermore,
we can view $GR$, $LR^{2}$, $\frac{G^{2}}{\mu}$ all as upper bounds
of the initial error $f(x^{(0)})-f^{*}$. For example, we know that
\[
...
\]
Therefore, the terms $\frac{GR}{\sqrt{k}}$, $\frac{G^{2}}{\mu k}$
and $\frac{LR^{2}}{k^{2}}$ are all just measuring how much we need
to decrease multiplicatively from the initial error. With this view,
we can think of the required numbers of iterations for Lipschitz convex
functions, strongly convex functions and smooth functions as exactly
$\frac{1}{\epsilon^{2}_{\mathrm{relative}}}$, $\frac{1}{\epsilon_{\mathrm{relative}}}$
and $\frac{1}{\sqrt{\epsilon_{\mathrm{relative}}}}$.
...
is 
\[
f(x)=\frac{1}{2}x^{2}_{1}+\frac{1}{2}\sum^{n-1}_{k=1}(x_{k}-x_{k+1})^{2}+\frac{1}{2}x^{2}_{n}
\]
This problem appears everywhere from physical systems and graph problems
to signal denoising and statistics. For this problem, we have $L=\Theta(1)$
...
When $x$ is close to the optimum $x^{*}=0$, gradient descent $x^{\new}=x-h\nabla f(x)$
is similar to a coordinate-wise iteration $x^{\new}_{i}=(1-h\lambda_{i})x_{i}$.
This gives a precise description of the convergence rate.

For more complicated algorithms, the theory of convergence rates and
per-iteration costs sometimes gives a poor estimate of practical performance.
...
rate; the simplex method is often the method of choice for solving
linear programs despite its exponential complexity bound.

The cost per iteration can also be misleading. Each iteration of interior
point methods takes $\nnz(A)+\mathrm{rank}(A)^{2}$ in theory while
it takes nearly linear time in practice for sparse matrices due to
the effectiveness of Cholesky decomposition. The $k^{\mathrm{th}}$
...
In Table \ref{tab:Black-Box-Problems}, we omitted many cases. For
example, what is the runtime if we have a stochastic function $\min_{x}\E_{\omega}f_{\omega}(x)$
such that $\E_{\omega}f_{\omega}(x)$ is strongly convex? What is
the runtime if we have a finite sum problem $\frac{1}{n}\sum_{i}f_{i}(x)$
which is not strongly convex? In this section, we show that one can
reduce runtimes between different cases. To highlight the idea, we
will be informal and ignore polylogarithmic terms. We note that some
of the reduction ideas are not just useful for proving theorems, but
are practical tricks to speed up algorithms and to avoid having to
know parameters. See \cite{lin2015universal,frostig2015regularizing,o2015adaptive,allen2016optimal}
...
\begin{itemize}
\item Output $\mathcal{A}(\widetilde{f},x^{(0)})$ where $\widetilde{f}(x)=f(x)+\frac{G}{R\sqrt{2k}}\|x\|^{2}_{2}.$
\end{itemize}
To understand the result, we let $f_{\sigma}(x)=f(x)+\frac{\sigma}{2}\|x\|^{2}_{2}$
...
for a fixed $k$, we choose $\sigma$ such that 
\[
...
\end{equation}
On the other hand, the strong convexity of $f$ shows that $f(x^{(i)})-\min_{x}f(x)\geq\frac{\mu}{2}\cdot\|x^{(i)}-x^{*}\|^{2}_{2}$.
Therefore, we have that
\[
\|x^{(i)}-x^{*}\|^{2}_{2}\leq2\sqrt{\frac{\epsilon}{\mu}}\|x^{(i-1)}-x^{*}\|_{2}.
\]
Therefore, if $\|x^{(i-1)}-x^{*}\|_{2}\geq8\sqrt{\frac{\epsilon}{\mu}}$,
then the distance to the optimum decreases by at least a factor of
two. After $\widetilde{O}(1)$ iterations, we have that $\|x^{(i-1)}-x^{*}\|_{2}=O\left(\sqrt{\frac{\epsilon}{\mu}}\right)$
