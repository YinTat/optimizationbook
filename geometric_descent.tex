...
By Theorem \ref{thm:gd_convex_strongly}, gradient descent converges
at the rate $(1-\frac{\mu}{L})^{k}$ in function value assuming the
function satisfies $\mu\cdot I\preceq\nabla^{2}f(x)\preceq L\cdot I$
...
\[
\frac{\mu}{2}\norm{(x^{*}-x)+\frac{1}{\mu}\nabla f(x)}^{2}_{2}-\frac{\norm{\nabla f(x)}^{2}_{2}}{2\mu}\le-\left(f(x)-f(x^{*})\right)
\]
Using $x^{+}\defeq x-\frac{1}{L}\nabla f(x)$ and $x^{++}\defeq x-\frac{1}{\mu}\nabla f(x)$,
we can write
\begin{equation}
\norm{x^{*}-x^{++}}^{2}_{2}\leq\frac{\norm{\nabla f(x)}^{2}_{2}}{\mu^{2}}-\frac{2}{\mu}(f(x)-f(x^{*})).\label{eq:geometricdescent_ball-1}
\end{equation}
...
\begin{align}
\norm{x^{*}-x^{++}}^{2}_{2} & \leq\frac{1}{\mu^{2}}\left(\norm{\nabla f(x)}^{2}_{2}-2\mu\cdot\left(f(x)-f(x^{*})\right)\right)\nonumber \\
 & \leq\frac{1}{\mu^{2}}\left(\norm{\nabla f(x)}^{2}_{2}-2\mu\cdot\left(f(x)-f(x^{+})\right)\right)-\frac{2}{\mu}\left(f(x^{+})-f(x^{*})\right)\nonumber \\
 & \leq\frac{1}{\mu^{2}}\left(\norm{\nabla f(x)}^{2}_{2}-2\mu\cdot\frac{1}{2L}\|\nabla f(x)\|^{2}_{2}\right)-\frac{2}{\mu}\left(f(x^{+})-f(x^{*})\right)\nonumber \\
 & \leq\frac{\left(1-\frac{\mu}{L}\right)}{\mu^{2}}\norm{\nabla f(x)}^{2}_{2}-\frac{2}{\mu}\left(f(x^{+})-f(x^{*})\right).\label{eq:radius-decrease}
\end{align}
...
Set $Q=\left\{ x\in\Rn:\norm{x-(x^{(k)}-\frac{1}{\mu}\nabla f(x^{(k)}))}^{2}_{2}\leq\frac{1-\frac{\mu}{L}}{\mu^{2}}\cdot\norm{\nabla f(x^{(k)})}^{2}_{2}\right\} $.
...
\[
Q^{(k+1)}=\mathtt{minSphere}\left(\left\{ \norm{x-(x^{(k)}-\frac{1}{\mu}\nabla f(x^{(k)}))}^{2}_{2}\leq\frac{1-\frac{\mu}{L}}{\mu^{2}}\cdot\norm{\nabla f(x^{(k)})}^{2}_{2}\right\} \cap\left\{ \|x-x^{(k)}\|\leq R^{(k)}\right\} \right).
\]
To compute $\frac{\mathrm{radius}(Q^{(k+1)})^{2}}{\mathrm{radius}(Q^{(k)})^{2}}$,
we can assume $x^{(k)}=0$ and $R^{(k)}=1$ and let $g=\frac{\nabla f(0)}{\mu}$.
Hence, we have
\[
Q^{(k+1)}=\mathtt{minSphere}\left(\left\{ \norm{x-g}^{2}_{2}\leq(1-\frac{\mu}{L})\cdot\norm g^{2}_{2}\right\} \cap\left\{ \|x\|\leq1\right\} \right).
\]
...
only one of the balls shrinks; the right diagram shows the intersection
...
or 
\[
\left\langle \nabla f(x),x^{*}-x\right\rangle <-(f(x)-f(x^{\new})).
\]
...
where $c^{(k+1)}$ is the minimizer of $q^{(k+1)}(y)$.
...
line search from $x^{(k)}$ along the descent direction $-\nabla f(x^{(k)})$.
...
\textbf{Input:} $q_{A}(x)=v_{A}+\frac{\mu}{2}\|x-c_{A}\|^{2}_{2}$
and $q_{B}(x)=v_{B}+\frac{\mu}{2}\|x-c_{B}\|^{2}_{2}$.
...
\textbf{Output: $q(x)=v_{\lambda}+\frac{\mu}{2}\|x-c_{\lambda}\|^{2}_{2}$.}
...
\]
This says that the quadratic lower bound improves significantly whenever
$\frac{\mu}{2}\|c_{A}-c_{B}\|^{2}\gg v_{B}-v_{A}$ or $\frac{\mu}{2}\|c_{A}-c_{B}\|^{2}\ll v_{B}-v_{A}$.
Using this, we can analyze the $\mathtt{ParabolaMethod}$.
\begin{thm}
\label{lem:geometric_convergence_rate-1}Assume that $f$ is $\mu$-strongly
convex with $L$-Lipschitz gradient. Let $r^{2}_{k}=f(x^{(k)})-\min_{y}q^{(k)}(y)$.
Then, we have that
\[
r^{2}_{k+1}\leq(1-\sqrt{\frac{\mu}{L}})\cdot r^{2}_{k}.
\]
...
Hence, $r^{2}_{k}$ measures the squared radius of our quadratic lower
...
Note that $r^{2}_{k+1}=f(x^{(k+1)})-v_{\lambda}$ and $r^{2}_{k}=f(x^{(k)})-v_{A}$.
Therefore, we have
\begin{equation}
\frac{r^{2}_{k}-r^{2}_{k+1}}{r^{2}_{k}}=\frac{f(x^{(k)})-f(x^{(k+1)})+v_{\lambda}-v_{A}}{r^{2}_{k}}.\label{eq:r_ratio-1}
\end{equation}
...
\[
v_{\lambda}\geq v_{A}+\frac{\mu}{8}\frac{(\frac{2}{\mu}(f(x^{(k)})-v_{A}))^{2}}{\frac{\|\nabla f(x^{(k)})\|^{2}}{\mu^{2}}}=v_{A}+\frac{\mu\cdot r^{4}_{k}}{2\|\nabla f(x^{(k)})\|^{2}}.
\]
Putting this into (\ref{eq:r_ratio-1}), we have
\begin{align}
\frac{r^{2}_{k}-r^{2}_{k+1}}{r^{2}_{k}} & \geq\frac{f(x^{(k)})-f(x^{(k+1)})}{r^{2}_{k}}+\frac{\mu\cdot r^{2}_{k}}{2\|\nabla f(x^{(k)})\|^{2}}\nonumber \\
 & \geq\frac{f(x^{(k)})-f(x^{(k+\frac{1}{2})})}{r^{2}_{k}}+\frac{\mu\cdot r^{2}_{k}}{2\|\nabla f(x^{(k)})\|^{2}}\nonumber \\
 & \geq\frac{\|\nabla f(x^{(k)})\|^{2}}{2Lr^{2}_{k}}+\frac{\mu\cdot r^{2}_{k}}{2\|\nabla f(x^{(k)})\|^{2}}\label{eq:geometric_key-1}\\
 & \geq\sqrt{\frac{\mu}{L}}\nonumber 
\end{align}
where the second inequality uses $f(x^{(k+1)})\leq f(x^{(k+\frac{1}{2})})$,
and the third inequality follows from the fact that $x^{(k+\frac{1}{2})}$
is a line search from $x^{(k)}$, together with the assumption on
$L$ (Lemma \ref{lem:gradient_progress}), the last inequality follows
...
and hence $r^{2}_{1}=f(x^{(1)})-v^{(1)}\leq\frac{1}{2\mu}\|\nabla f(x^{(0)})\|^{2}$.
...
There exists $x$ such that $B(0,\sqrt{1-\epsilon|g|^{2}})\cap B(g,|g|\sqrt{1-\epsilon})\subset B(x,\sqrt{1-\sqrt{\epsilon}})$.
\end{xca}
...
a careful implementation, line search can be as cheap as a gradient
