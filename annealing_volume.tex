...
\SetAlgoLined
\begin{enumerate}
\item For $i=0,\ldots,m=\lceil\sqrt{n}\ln(4R^{2}n/\varepsilon)\rceil$,
let $a_{i}=2n/(1+\frac{1}{\sqrt{n}})^{i}$ and $f_{i}=f(x)^{a_{i}}.$
...
\end{enumerate}
\item Let $Y=\prod^{m-1}_{i=0}Y_{i}$ and return $Y_{0}\cdot Y$.
\end{enumerate}
\end{algorithm2e}
