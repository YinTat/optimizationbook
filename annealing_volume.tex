...
\end{enumerate}
\item Let $Y=\prod^{m-1}_{i=1}Y_{i}$ and return $Y\cdot(\pi/n)^{n/2}$.
\end{enumerate}
\end{algorithm2e}
...
\[
\frac{F(2a_{i+1}-a_{i})F(a_{i})}{F(a_{i+1})^{2}}\le\left(\frac{a^{2}_{i+1}}{(2a_{i+1}-a_{i})a_{i}}\right)^{n}.
\]
...
Gaussian cooling utilizes the fact that the KLS conjecture holds for
a Gaussian density restricted by any convex body, and completely avoids
computing an isotropic transformation.
...
\item For $i=0,\ldots,m-1$, let $a_{i}=\frac{1}{4n}(1+\frac{1}{\sqrt{n}})^{i},\,a_{m}=1$
and $f_{i}=f(x)^{a_{i}}$, with $m-1$ being the largest integer s.t.
$a_{m-1}<1$. 
\item For $i=0,\ldots,m-1$, 
\begin{enumerate}
\item sample $X^{(1)},X^{(2)},\ldots,X^{(N)}$ from $\pi_{f_{i}}$.
\item Set $Y_{i}=\frac{1}{N}\sum^{N}_{j=1}f(x)^{a_{i+1}-a_{i}}$
\end{enumerate}
\item Let $Y=\prod^{m-1}_{i=1}Y_{i}$ and return $Y_{o}\cdot Y$.
\end{enumerate}
\end{algorithm2e}
