...
\]
\end{proof}
To make the formula simpler, we define $p^{(k)}=\frac{\|r^{(k-1)}\|^{2}_{2}}{v^{(k)\top}r^{(k-1)}}v^{(k)}$.
\begin{lem}
We have that $x^{(k)}=x^{(k-1)}+\frac{\|r^{(k-1)}\|^{2}_{2}}{p^{(k)\top}Ap^{(k)}}p^{(k)}$
and $p^{(k)}=r^{(k-1)}-\frac{\|r^{(k-1)}\|^{2}}{\|r^{(k-2)}\|^{2}_{2}}p^{(k-1)}.$
\end{lem}

\begin{proof}
For the first formula, note that
\[
x^{(k)}=x^{(k-1)}+v^{(k)}=x^{(k-1)}+\frac{v^{(k)\top}r^{(k-1)}}{\|r^{(k-1)}\|^{2}_{2}}p^{(k)}.
\]
For the quantity $v^{(k)\top}r^{(k-1)}$, we note that $f(x^{(k-1)}+tv^{(k)})$
is minimized at $t=1$ and hence
\begin{align*}
v^{(k)\top}r^{(k-1)} & =v^{(k)\top}Av^{(k)}\\
 & =\frac{v^{(k)\top}r^{(k-1)}}{\|r^{(k-1)}\|^{2}_{2}}\frac{v^{(k)\top}r^{(k-1)}}{\|r^{(k-1)}\|^{2}_{2}}p^{(k)\top}Ap^{(k)}
\end{align*}
where we used the definition of $p^{(k)}$. Simplifying gives
\[
\frac{v^{(k)\top}r^{(k-1)}}{\|r^{(k-1)}\|^{2}_{2}}=\frac{\|r^{(k-1)}\|^{2}_{2}}{p^{(k)\top}Ap^{(k)}}
\]
and hence the first formula.

For the second formula, we use Lemma \ref{lem:CG_v} and substitute
$p^{(k)}=\frac{\|r^{(k-1)}\|^{2}_{2}}{v^{(k)\top}r^{(k-1)}}v^{(k)}$.
This gives
\begin{equation}
p^{(k)}=r^{(k-1)}-\frac{r^{(k-1)\top}Av^{(k-1)}}{\|r^{(k-2)}\|^{2}_{2}}p^{(k-1)}.\label{eq:CG_p_temp}
\end{equation}
Note that
\[
...
$\alpha=\frac{\|r^{(k-1)}\|^{2}_{2}}{p^{(k)\top}Ap^{(k)}}$.
...
have
\[
f(x^{(k)})-f(x^{*})\leq\frac{1}{2}\inf_{q(0)=1}\max_{i}q(\lambda_{i})^{2}\cdot\|b\|^{2}_{A^{-1}}
\]
...
2(f(x^{(k)})-f(x^{*})) & =\inf_{\deg q\leq k,q(0)=1}\|q(A)A^{-\frac{1}{2}}b\|^{2}_{2}\\
 & \leq\inf_{q(0)=1}\|q(A)\|^{2}_{\op}\cdot\|b\|^{2}_{A^{-1}}.
\end{align*}
