...
\]
Since $f$ is $\mu+\frac{L}{n}$ smooth and $\mu$ strongly convex
and since $h$ is convex, we apply Theorem \ref{thm:AGD} and get
an algorithm that takes $O^{*}(\sqrt{\frac{L}{n\mu}})$ steps. Note
that each step involves $\mathcal{T}_{f}+\mathcal{T}_{h,\mu+\frac{L}{n}}$.
...
function $\ell$. Suppose that $\frac{\partial^{2}}{\partial x^{2}_{i}}\ell(x)\leq L_{i}$
...
Since $f$ is $\mu+\frac{L}{n}$ smooth and $\mu$ strongly convex
and since $h$ is convex, we apply Theorem \ref{thm:AGD} and get
an algorithm that takes $O^{*}(1+\sqrt{\frac{L}{n\mu}})$ steps. Note
that each step involves $\mathcal{T}_{f}+\mathcal{T}_{h,\mu+\frac{L}{n}}$.
Obviously, $\mathcal{T}_{f}=0$. Next, note that $\mathcal{T}_{h,\mu+\frac{L}{n}}$
involves solving a problem of the form
\begin{align*}
