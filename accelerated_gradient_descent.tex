...
According to legend, Nemirovski gave the first proof of accelerated
gradient descent in the 1970s; it used a $2$-dimensional plane search
subroutine. Later, this was improved to line search, and in 1982 Nesterov
showed how to remove line search. We modify the proof slightly to
avoid tuning parameters.
...
$P^{(k)}\leftarrow x^{(1)}+\mathrm{span}(x^{(k)+}-x^{(1)},\sum^{k}_{s=1}\frac{\nabla f(x^{(s)})}{\|\nabla f(x^{(s)})\|})$.

\tcp{Alternatively, one can use $P^{(k)}\leftarrow x^{(k)}+\mathrm{span}(x^{(k)}-x^{(1)},\nabla f(x^{(k)}),\sum^{k}_{s=1}\frac{\nabla f(x^{(s)})}{\|\nabla f(x^{(s)})\|})$
...
\[
\sum^{T}_{k=1}\lambda_{k}\delta_{k}\leq\left\langle \sum^{T}_{k=1}\frac{\nabla f(x^{(k)})}{\|\nabla f(x^{(k)})\|},x^{(1)}-x^{*}\right\rangle \leq\|x^{*}-x^{(1)}\|_{2}\cdot\norm{\sum^{T}_{k=1}\frac{\nabla f(x^{(k)})}{\|\nabla f(x^{(k)})\|}}_{2}.
\]
Finally, we note that $\nabla f(x^{(k+1)})\perp P^{(k)}$ and hence
$\nabla f(x^{(k+1)})\perp\sum^{k}_{s=1}\frac{\nabla f(x^{(s)})}{\|\nabla f(x^{(s)})\|}$.
Therefore, we have 
\[
\norm{\sum^{T}_{k=1}\frac{\nabla f(x^{(k)})}{\|\nabla f(x^{(k)})\|}}^{2}_{2}=\sum^{T}_{k=1}\norm{\frac{\nabla f(x^{(k)})}{\|\nabla f(x^{(k)})\|}}^{2}_{2}=T.
\]
Hence, we have
\[
\sum^{T}_{k=1}\lambda_{k}\delta_{k}\leq\sqrt{T}\cdot\|x^{*}-x^{(1)}\|_{2}.
\]
Since $x^{(k)+}\in P^{(k)}$, we have that 
\[
\delta_{k+1}=f(x^{(k+1)})-f(x^{*})\leq f(x^{(k)+})-f(x^{*})\leq\delta_{k}-\frac{1}{2L}\|\nabla f(x^{(k)})\|^{2}_{2}.
\]

Hence, we have $\|\nabla f(x^{(k)})\|^{2}_{2}\leq2L(\delta_{k}-\delta_{k+1})$.
This gives
\[
\sum^{T}_{k=1}\frac{\delta_{k}}{\sqrt{\delta_{k}-\delta_{k+1}}}\leq\sqrt{2LT}\cdot\|x^{*}-x^{(1)}\|_{2}
\]
...
Note that the proof above used only the fact that $\{x^{(1)},x^{(k)+},\sum^{k}_{s=1}\frac{\nabla f(x^{(s)})}{\|\nabla f(x^{(s)})\|}\}\subset P^{(k)}$.
...
\]
then one can prove that this algorithm is equivalent to conjugate
...
To give a general version of acceleration, we consider a problem of
the form
\[
...
To handle this issue, we need to define an approximate gradient of
$\phi$ that we can compute.
\begin{defn}
...
that is computable in time $\mathcal{T}_{f}+\mathcal{T}_{h,L}$.

The key lemma we use here is that $\phi$ satisfies a lower bound
defined using $g_{x}$. Ideally, we would love to get a lower bound
as follows:
\[
\phi(z)\geq\phi(x)+g^{\top}_{x}(z-x)+\frac{\mu}{2}\norm{z-x}^{2}_{2}.
\]
However, this inequality is false. If that was true for all $z$,
then we would have $g_{x}=\nabla\phi(x)$. However, if $\phi\in\mathcal{C}^{2}$
is $\mu$ strongly convex, then we have
\begin{align}
\phi(z) & \geq\phi(x)+\nabla\phi(x)^{\top}(z-x)+\frac{\mu}{2}\norm{z-x}^{2}_{2}\nonumber \\
 & \geq\phi(x-\frac{1}{L}\nabla\phi(x))+\frac{1}{2L}\norm{\nabla\phi(x)}^{2}+\nabla\phi(x)^{\top}(z-x)+\frac{\mu}{2}\norm{z-x}^{2}_{2}.\label{eq:gradient_lower_bound}
\end{align}
...
\[
\phi(z)\geq\phi(p_{x})+g^{\top}_{x}(z-x)+\frac{1}{2L}\norm{g_{x}}^{2}_{2}+\frac{\mu}{2}\norm{z-x}^{2}_{2}.
\]
\end{thm}
...
h(z) & \geq h(p_{x})+\nabla f(x)^{\top}(p_{x}-z)+L(p_{x}-x)^{\top}(p_{x}-z)\\
 ...
 & =h(p_{x})+\nabla f(x)^{\top}(p_{x}-z)+\frac{1}{L}\|g_{x}\|^{2}+g^{\top}_{x}(z-x).
\end{align*}
Using that 
\[
...
we have the result:
\begin{align*}
\phi(z) & \geq h(p_{x})+f(x)+\nabla f(x)^{\top}(p_{x}-x)+\frac{1}{L}\|g_{x}\|^{2}+g^{\top}_{x}(z-x)+\frac{\mu}{2}\|z-x\|^{2}\\
 & \geq\phi(p_{x})+\frac{1}{2L}\|g_{x}\|^{2}+g^{\top}_{x}(z-x)+\frac{\mu}{2}\|z-x\|^{2}.
\end{align*}
...
\]
Let $s\in\partial h(x)$. Using $h(p_{x})\geq h(x)+s^{\top}(p_{x}-x)$
and $\nabla f(x)+s\in\partial\phi(x)$, we have that
\[
0\geq(\nabla f(x)+s)^{\top}(p_{x}-x)+\frac{L}{2}\norm{p_{x}-x}^{2}\geq-G\norm{p_{x}-x}_{2}+\frac{L}{2}\norm{p_{x}-x}^{2}.
\]
...
value by $\frac{1}{2L}\norm{g_{x}}^{2}_{2}$. Therefore, if the gradient
is large, then the function value decreases substantially. On the
other hand, putting $z=x^{*}$ for Theorem \ref{thm:gradient_mapping_lower_bound}
shows that
\[
\phi(x^{*})\geq\phi(p_{x})+g^{\top}_{x}(x^{*}-x).
\]

If the gradient is small and the domain is bounded, this shows that
we are close to the optimum. Combining these two facts, we can get
the gradient descent.

\subsection{Mirror Descent Using Gradient Mapping}

In this subsection, the mirror map is the Euclidean quadratic, so
the mirror descent update has the same form as a gradient step with
the gradient mapping in place of the gradient. Consider the mirror
descent as
\begin{equation}
...
of $\phi$ to get that
\begin{equation}
\phi(\frac{1}{k}\sum^{k}_{i=1}p_{x^{(i)}})-\phi(x^{*})\leq\frac{1}{k}\sum^{k}_{i=1}(\phi(p_{x^{(i)}})-\phi(x^{*}))\leq\frac{1}{k}\sum^{k}_{i=1}g^{\top}_{x^{(i)}}(x^{(i)}-x^{*}).\label{eq:mirror_lemma}
\end{equation}
Therefore, it suffices to upper bound $g^{\top}_{x^{(i)}}(x^{(i)}-x^{*})$.
The following lemma shows that if $g^{\top}_{x^{(i)}}(x^{(i)}-x^{*})$
is large, then either the gradient is large or the distance to the
optimum changes substantially. It turns out this holds for any vector
$g$, not necessarily an approximate gradient.
\begin{lem}[Mirror Descent Lemma]
\label{lem:mirror_step}Let $p=x-\eta g$. Then, we have that
\[
g^{\top}(x-u)=\frac{\eta}{2}\norm g^{2}_{2}+\frac{1}{2\eta}\left(\norm{x-u}^{2}_{2}-\norm{p-u}^{2}_{2}\right)
\]
for any $u$.
\end{lem}
...
is large, gradient descent makes large progress. On the other hand,
if the gradient is small, (\ref{eq:mirror_lemma}) shows that mirror
descent makes large progress. Therefore, it is natural to combine
two approaches.
...
\textbf{Return $\frac{1}{T}\sum^{T}_{k=1}p_{x^{(k+1)}}$.}
...
we have that
\[
\phi(\frac{1}{T}\sum^{T}_{k=1}p_{x^{(k+1)}})-\phi(x^{*})\leq\frac{2}{T}\sqrt{\frac{L}{\mu}}\left(\phi(x^{(1)})-\phi(x^{*})\right).
\]
...
\begin{align}
g^{\top}_{x^{(k+1)}}(z^{(k)}-x^{*}) & \leq\frac{\eta}{2}\norm{g_{x^{(k+1)}}}^{2}_{2}+\frac{1}{2\eta}\left(\norm{z^{(k)}-x^{*}}^{2}_{2}-\norm{z^{(k+1)}-x^{*}}^{2}_{2}\right)\label{eq:AGD_1}
\end{align}

This shows that if the mirror descent has large error $g^{\top}_{x^{(k+1)}}(z^{(k)}-x^{*})$,
then the gradient descent makes large progress ($\frac{\eta}{2}\norm{g_{x^{(k+1)}}}^{2}_{2}$).

To make the left-hand side usable, note that $x^{(k+1)}=z^{(k)}+\frac{1-\tau}{\tau}\cdot(y^{(k)}-x^{(k+1)})$
and hence
\begin{align*}
g^{\top}_{x^{(k+1)}}(x^{(k+1)}-x^{*}) & =g^{\top}_{x^{(k+1)}}(z^{(k)}-x^{*})+\frac{1-\tau}{\tau}\cdot g^{\top}_{x^{(k+1)}}(y^{(k)}-x^{(k+1)})\\
 & \leq g^{\top}_{x^{(k+1)}}(z^{(k)}-x^{*})+\frac{1-\tau}{\tau}(\phi(y^{(k)})-\phi(y^{(k+1)})-\frac{1}{2L}\norm{g_{x^{(k+1)}}}^{2}_{2})\\
 & \leq\frac{\eta}{2}\norm{g_{x^{(k+1)}}}^{2}_{2}+\frac{1}{2\eta}\left(\norm{z^{(k)}-x^{*}}^{2}_{2}-\norm{z^{(k+1)}-x^{*}}^{2}_{2}\right)+\frac{1-\tau}{\tau}(\phi(y^{(k)})-\phi(y^{(k+1)})-\frac{1}{2L}\norm{g_{x^{(k+1)}}}^{2}_{2}).
\end{align*}
...
\[
g^{\top}_{x^{(k+1)}}(x^{(k+1)}-x^{*})\leq\eta L(\phi(y^{(k)})-\phi(y^{(k+1)}))+\frac{1}{2\eta}\left(\norm{z^{(k)}-x^{*}}^{2}_{2}-\norm{z^{(k+1)}-x^{*}}^{2}_{2}\right).
\]
Summing over all $k$ and letting $\overline{x}=\frac{1}{T}\sum^{T}_{k=1}p_{x^{(k+1)}}$,
we have that
\begin{align*}
\phi(\overline{x})-\phi(x^{*}) & \leq\frac{1}{T}\sum^{T}_{k=1}(\phi(p_{x^{(k+1)}})-\phi(x^{*}))\\
 & \leq\frac{1}{T}\sum^{T}_{k=1}g^{\top}_{x^{(k+1)}}(x^{(k+1)}-x^{*})\\
 & \leq\frac{\eta L}{T}\left(\phi(y^{(1)})-\phi(y^{(T+1)})\right)+\frac{1}{2\eta T}\norm{z^{(1)}-x^{*}}^{2}_{2}\\
 & \leq\left(\frac{\eta L}{T}+\frac{1}{\eta T\mu}\right)\left(\phi(x^{(1)})-\phi(x^{*})\right)
\end{align*}
...
Recall from Theorem \ref{thm:coordinate_descent} that if $\frac{\partial^{2}}{\partial x^{2}_{i}}\ell(x)\leq L_{i}$
for all $x$ and that $\ell$ is $\mu$ strongly convex, then we can
find $x^{(k)}$ in $k$ coordinate steps such that 
\[
\E\ell(x^{(k)})-\ell(x^{*})\leq(1-\frac{\mu}{L})^{\Omega(k)}(\ell(x^{(0)})-\ell(x^{*}))
\]
where $L=\sum_{i}L_{i}$. Now, the really fun part is here. Consider
the function 
\[
\phi(x)=f(x)+h(x)\quad\text{with}\quad f(x)=\frac{\mu}{2}\norm x^{2}_{2}\text{ and }h(x)=\ell(x)-\frac{\mu}{2}\norm x^{2}_{2}.
\]
Since $f$ is $\mu$-smooth and $\mu$-strongly convex, and since
$h$ is convex, we apply Theorem \ref{thm:AGD} using the valid smoothness
upper bound $\mu+\frac{L}{n}$ and get an algorithm that takes $O^{*}(\sqrt{\frac{L}{n\mu}})$
...
\label{thm:coordinate_descent_linear_acc}Let $\ell$ be a $\mu$-strongly
convex function. Suppose that $\frac{\partial^{2}}{\partial x^{2}_{i}}\ell(x)\leq L_{i}$
...
Recall from Theorem \ref{thm:stochastic_method} that if $\nabla^{2}\ell_{i}(x)\preceq L\cdot I$
for all $x$ and $i$ and that $\ell(x)=\frac{1}{n}\sum^{n}_{i=1}\ell_{i}(x)$
...
Since $f$ is $\mu$-smooth and $\mu$-strongly convex, and since
$h$ is convex, we apply Theorem \ref{thm:AGD} using the valid smoothness
upper bound $\mu+\frac{L}{n}$ and get an algorithm that takes $O^{*}(1+\sqrt{\frac{L}{n\mu}})$
steps. Note that each step involves $\mathcal{T}_{f}+\mathcal{T}_{h,\mu+\frac{L}{n}}$.
Obviously, $\mathcal{T}_{f}=0$. Next, note that $\mathcal{T}_{h,\mu+\frac{L}{n}}$
involves solving a problem of the form
\begin{align*}
...
Suppose that $\nabla^{2}\ell_{i}(x)\preceq L\cdot I$ for all $i$
and $x$ and that $\ell$ is $\mu$ strongly convex. Suppose we can
compute $\nabla\ell_{i}$ in $O(1)$ time. We have an algorithm that
outputs an $x$ such that 
\[
