...
i.e., one that does not satisfy some smoothness condition which directly
...
\[
\min_{x}\sum^{m}_{i=1}f_{i}(x)=-\min_{\theta_{1},\ldots,\theta_{m-1}}\left(f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1})\right)
\]
\end{lem}
...
\begin{align*}
f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1}) & \ge((-\theta_{1})^{\top}x-f_{1}(x))+\sum^{m-1}_{i=2}((\theta_{i-1}-\theta_{i})^{\top}x-f_{i}(x))\\
 & ~+(\theta^{\top}_{m-1}x-f_{m}(x))\\
 & =-\sum^{m}_{i=1}f_{i}(x)\\
\sum^{m}_{i=1}f_{i}(x) & \ge-(f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1}))
\end{align*}
So, 
\begin{align*}
\inf_{x}\sum^{m}_{i=1}f_{i}(x) & \ge\sup_{\theta_{1},\ldots,\theta_{m-1}}-(f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1}))
\end{align*}
It remains to show that there exist $x$ and $\theta_{1},\ldots,\theta_{m-1}$
such that 
\[
\sum^{m}_{i=1}f_{i}(x)\le-(f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1}))
\]
($\ge$ is guaranteed by the preceding inequality). We choose $x\in\R^{n}$
to minimize the convex function $\sum^{m}_{i=1}f_{i}(x)$. Then, $0\in\nabla(\sum^{m}_{i=1}f_{i}(x))=\sum^{m}_{i=1}\nabla f_{i}(x)$
(as Minkowski sum of subgradients), so there exists $\alpha_{1},\ldots,\alpha_{m}\in\R^{n}$
such that $\sum^{m}_{i=1}\alpha_{i}=0$ and $\alpha_{i}\in\nabla f_{i}(x)$.
By definition of subgradient, for all $y\in\R^{n}$, 
\begin{align*}
f_{i}(y) & \ge f_{i}(x)+\alpha^{\top}_{i}(y-x)\\
\alpha^{\top}_{i}y-f_{i}(y) & \le\alpha^{\top}_{i}x-f_{i}(x)\\
f^{*}_{i}(\alpha_{i}) & =\sup_{y}\alpha^{\top}_{i}y-f_{i}(y)\\
 & \le\alpha^{\top}_{i}x-f_{i}(x)
\end{align*}
Then, 
\begin{align*}
\sum^{m}_{i=1}f^{*}_{i}(\alpha_{i}) & \le\sum^{m}_{i=1}\left(\alpha^{\top}_{i}x-f_{i}(x)\right)\\
 & =-\sum^{m}_{i=1}f_{i}(x)
\end{align*}
Finally, we choose $\theta_{i}=-\sum^{i}_{j=1}\alpha_{j}$ for $i=1,\ldots,m-1$,
so that $\alpha_{i}=\theta_{i-1}-\theta_{i}$, and $\theta_{0}=\theta_{m}=0$.
Then, 
\[
\sum^{m}_{i=1}f_{i}(x)\le-\sum^{m}_{i=1}f^{*}_{i}(\theta_{i-1}-\theta_{i})
\]
as desired.
\end{proof}
\begin{rem*}
...
to the gradient, as the only terms with $\theta_{i}$ are $f^{*}_{i}(\theta_{i-1}-\theta_{i}),f^{*}_{i+1}(\theta_{i}-\theta_{i+1})$
\end{rem*}
...
the vertex-edge incidence matrix with two nonzeros per column, $d$
...
problem with the cut, for a threshold $\tau$, given by $\{v\in V:\phi(v)\geq\tau\}$.
...
Here, we use semidefinite programming as a concrete example. Define
$\bullet$ as the inner product of matrices, i.e., 
\[
...
and its dual
\begin{equation}
\min_{y}b^{\top}y\text{ s.t. }\sum^{m}_{i=1}y_{i}A_{i}\succeq C\label{eq:dual_SDP}
\end{equation}
...
cutting plane method naively on the primal problem, we would get $\tilde{O}(n^{2}(Z+n^{4}))$-time
algorithm for the primal (because there are $n^{2}$ variables) and
$\tilde{O}(m(Z+n^{\omega}+m^{2}))$ for the dual where $Z$ is the
total number of non-zeros in $A_{i}$. (The term $Z+n^{\omega}$ is
the cost of computing $\sum y_{i}A_{i}$ and finding its minimum eigenvalue.)
Generally, $n^{2}\gg m$ and hence it takes much less time to solve
the dual.

We note that
\[
\min_{\sum^{m}_{i=1}y_{i}A_{i}\succeq C}b^{\top}y=\min_{v^{\top}(\sum^{m}_{i=1}y_{i}A_{i}-C)v\geq0\ \forall\norm v_{2}=1}b^{\top}y.
\]
In each step of the cutting plane method, the (sub)gradient oracle
either outputs $b$ or outputs one of the cutting planes
\[
v^{\top}(\sum^{m}_{i=1}y_{i}A_{i}-C)v\geq0.
\]
Let $\mathcal{S}$ be the set of vectors whose cutting planes are
used in the algorithm. Then, the proof of the cutting plane method
shows that
\begin{equation}
\min_{\sum^{m}_{i=1}y_{i}A_{i}\succeq C}b^{\top}y=\min_{v^{\top}(\sum^{m}_{i=1}y_{i}A_{i}-C)v\geq0\ \forall v\in\mathcal{S}}b^{\top}y\pm\varepsilon.\label{eq:SDP_apx}
\end{equation}
...
problem). Now, we have
\begin{align*}
\min_{v^{\top}(\sum^{m}_{i=1}y_{i}A_{i}-C)v\geq0\ \forall v\in\mathcal{S}}b^{\top}y & =\min_{y}\max_{\lambda_{v}\geq0}b^{\top}y-\sum_{v\in\mathcal{S}}\lambda_{v}v^{\top}(\sum^{m}_{i=1}y_{i}A_{i}-C)v\\
 & =\max_{\lambda_{v}\geq0}\min_{y}C\bullet\sum_{v\in\mathcal{S}}\lambda_{v}vv^{\top}+b^{\top}y-\sum^{m}_{i=1}y_{i}\sum_{v\in\mathcal{S}}\lambda_{v}vv^{\top}\bullet A_{i}\\
 & =\max_{X=\sum_{v\in\mathcal{S}}\lambda_{v}vv^{\top},\lambda_{v}\geq0}\min_{y}C\bullet X+\sum^{m}_{i=1}y_{i}(b_{i}-X\bullet A_{i})\\
...
method involves finding a separating hyperplane of $\sum^{m}_{i=1}y_{i}A_{i}\succeq C$. 
\begin{xca}
Let $\Omega=\{y\in\Rm:\ \sum^{m}_{i=1}y_{i}A_{i}\succeq C\}$. Show
...
that every student can be assigned to at most one school and school
$b$ can accept $c_{b}$ students. Then, the problem can be formulated
as
\[
\max_{x_{e}\geq0}\sum_{e\in E}w_{e}x_{e}\text{ subject to }\sum_{b:(a,b)\in E}x_{(a,b)}\leq1\ \forall a\in V_{1},\sum_{a:(a,b)\in E}x_{(a,b)}\leq c_{b}\ \forall b\in V_{2}.
\]
This is the weighted b-matching problem. Typically, the number of
students is much larger than the number of schools. Therefore, an
algorithm with running time linear in the number of students is preferable.
To apply our framework, we let
\begin{align*}
K_{1} & =\{x\in\R^{E},x_{e}\geq0,\sum_{b:(a,b)\in E}x_{(a,b)}\leq1\ \forall a\in V_{1}\},\\
K_{2} & =\{y\in\R^{V_{2}},y_{b}\leq c_{b}\},
\end{align*}
and $M:\R^{E}\rightarrow\R^{V_{2}}$ is the map $(Mx)_{b}=\sum_{a:(a,b)\in E}x_{(a,b)}.$
...
$K_{2}=\{y\in\R^{V}:\sum_{i\in S}y_{i}\leq f(S)\text{ for all }S\subset V\}$,
$M$ is the incidence matrix.
\end{itemize}
In all of these examples, it is easy to compute the gradient of $\delta^{*}_{K_{1}}$
and $\delta^{*}_{K_{2}}$. For the last three examples, it is not
clear how to compute the gradient of $\delta_{K_{1}}$ and/or $\delta_{K_{2}}$
...
between $\nabla\delta^{*}_{K_{1}}$ and $\nabla\delta_{K_{1}}$, and
then apply cutting plane methods. With the running time of cutting
plane methods, such an algorithm usually had theoretical running time
at least $n^{5}$ and was of little practical value. 
...
\min_{x\in K_{1},Mx\in K_{2}}c^{\top}x & =\min_{x\in\Rn}c^{\top}x+\delta_{K_{1}}(x)+\delta_{K_{2}}(Mx)\\
 & =\min_{x\in\Rn}\max_{\theta\in\Rm}c^{\top}x+\delta_{K_{1}}(x)+\theta^{\top}Mx-\delta^{*}_{K_{2}}(\theta)\\
 & =\max_{\theta\in\Rm}\min_{x\in\Rn}c^{\top}x+\delta_{K_{1}}(x)+\theta^{\top}Mx-\delta^{*}_{K_{2}}(\theta)\\
 & =\max_{\theta\in\Rm}-\delta^{*}_{K_{1}}(-c-M^{\top}\theta)-\delta^{*}_{K_{2}}(\theta).
\end{align*}
...
and $\nabla\delta^{*}_{K_{2}}$. The only problem left is to recover
the primal $x$.
...
\begin{lem}
Let $\{x_{i}\}\subset K_{1}$ be the set of points output by the oracle
$\nabla\delta^{*}_{K_{1}}$ during the cutting plane method. Define
$\{y_{i}\}\subset K_{2}$ similarly. Suppose that the cutting plane
method ends with the guarantee that the additive error is less than
$\varepsilon$. Then, we have that
\[
\min_{x\in K_{1},Mx\in K_{2}}c^{\top}x\leq\min_{x\in\widetilde{K_{1}},Mx\in\widetilde{K_{2}}}c^{\top}x\leq\min_{x\in K_{1},Mx\in K_{2}}c^{\top}x+\varepsilon
\]
...
$\nabla\delta^{*}_{K_{1}}$ and $\varphi_{i}$ be the directions queried
by the oracle for $\nabla\delta^{*}_{K_{2}}$. We claim that $x_{i}\in\nabla\delta^{*}_{\widetilde{K_{1}}}(\theta_{i})$
and $y_{i}\in\nabla\delta^{*}_{\widetilde{K_{2}}}(\varphi_{i})$.
...
\[
\min_{x\in\widetilde{K_{1}},Mx\in\widetilde{K_{2}}}c^{\top}x\leq c^{\top}x\leq\min_{x\in K_{1},Mx\in K_{2}}c^{\top}x+\varepsilon.
\]

To prove the claim, we note that $x_{i}\in\nabla\delta^{*}_{\widetilde{K_{1}}}(\theta_{i})$.
Note that $\widetilde{K_{1}}\subset K_{1}$ and hence $\max_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x\leq\max_{x\in K_{1}}\theta^{\top}_{i}x$.
Also, note that 
\[
x_{i}=\arg\max_{x\in K_{1}}\theta^{\top}_{i}x\in\widetilde{K_{1}}
\]
and hence $\max_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x\geq\max_{x\in K_{1}}\theta^{\top}_{i}x$.
Therefore, we have that $\max_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x=\max_{x\in K_{1}}\theta^{\top}_{i}x$.
Therefore, $x_{i}\in\arg\max_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x$.
This proves the claim for $\widetilde{K_{1}}$. The proof for $\widetilde{K_{2}}$
is the same.
\end{proof}
This reduces the problem into the form $\min_{x\in\widetilde{K_{1}},Mx\in\widetilde{K_{2}}}c^{\top}x$.
For the second phase, we let $z_{i}=Mx_{i}\in\Rm$. Then, we have
\begin{align*}
\min_{x\in\widetilde{K_{1}},Mx\in\widetilde{K_{2}}}c^{\top}x= & \min_{\substack{t_{i},s_{i}\geq0,\ \sum_{i}t_{i}=\sum_{i}s_{i}=1\\
M\sum_{i}t_{i}x_{i}=\sum_{i}s_{i}y_{i}
}
}c^{\top}(\sum_{i}t_{i}x_{i})\\
= & \min_{\substack{t_{i},s_{i}\geq0,\ \sum_{i}t_{i}=\sum_{i}s_{i}=1\\
\sum_{i}t_{i}z_{i}=\sum_{i}s_{i}y_{i}
}
}\sum_{i}t_{i}\cdot c^{\top}x_{i}.
\end{align*}
...
into two parts, where both are easy to solve and one part has fewer
variables, then we can solve the entire problem in time depending
on the smaller dimension.
\begin{xca}
How fast can we solve $\min_{x\in\cap^{k}_{i=1}K_{i}}c^{\top}x$ given
the oracles $\nabla\delta^{*}_{K_{i}}$?
\end{xca}
