...
\begin{align*}
f^{*}_{1}(-\theta_{1})+\sum^{m-1}_{i=2}f^{*}_{i}(\theta_{i-1}-\theta_{i})+f^{*}_{m}(\theta_{m-1}) & \ge((-\theta_{1})^{\top}x-f_{1}(x))+\sum^{m-1}_{i=1}((\theta_{i-1}-\theta_{i})^{\top}x-f_{i}(x))\\
...
the convex function $\sum^{m}_{i=1}f_{i}(x)$. Then, $0\in\nabla(\sum^{m}_{i=1}f_{i}(x))=\sum^{m}_{i=1}\nabla f_{i}(x)$
...
\sum^{m}_{i=1}f^{*}_{i}(\alpha_{i}) & \le\sum^{m}_{i=1}\left(\alpha^{\top}_{i}x-f_{i}(x)\right)\\
 & =-\sum^{m}_{i=1}f_{i}(x)
\end{align*}
Finally, we choose $\theta_{i}=-\sum^{i}_{j=1}\alpha_{j}$ for $i=0,\ldots,m$,
...
Note that $\widetilde{K_{1}}\subset K_{1}$ and hence $\min_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x\geq\min_{x\in K_{1}}\theta^{\top}_{i}x$.
Also, note that 
\[
x_{i}=\arg\min_{x\in K_{1}}\theta^{\top}_{i}x\in\widetilde{K_{1}}
\]
and hence $\min_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x\leq\min_{x\in K_{1}}\theta^{\top}_{i}x$.
Therefore, we have that $\min_{x\in\widetilde{K_{1}}}\theta^{\top}_{i}x=\min_{x\in K_{1}}\theta^{\top}_{i}x$.
Therefore, $x_{i}\in\arg\min_{x\in K_{1}}\theta^{\top}_{i}x$. This
