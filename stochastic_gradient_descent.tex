...
In this section, we study stochastic objectives, including expectations
$F(x)=\E_{f\sim\mathcal{D}}f(x)$ and finite sums $F(x)=\frac{1}{n}\sum^{n}_{i=1}f_{i}(x)$,
where each sampled or component function is convex.
...
\[
F_{T}(x)\defeq\frac{1}{T}\sum^{T}_{i=1}f_{i}(x),\qquad x^{(T)}_{\mathrm{ERM}}=\arg\min_{x}F_{T}(x).
\]
...
$h=\frac{1}{64L}$ and $m=\frac{512L}{\mu}$, the sequence $x^{(km)}$
...
\begin{align*}
\frac{1}{m}\sum^{m-1}_{k=0}\E F(x^{(k)})-F(x^{*}) & \leq16hL\cdot(F(x^{(0)})-F(x^{*}))+\frac{2(F(x^{(0)})-F(x^{*}))}{\mu hm}\\
...
\textbf{Input:} Initial point $x^{(0)}\in\R^{d}$, coordinate smoothness
constants $L_{i}>0$.
...
that $\frac{\partial^{2}}{\partial x^{2}_{i}}f(x)\leq L_{i}$ for
all $x$ and let $L=\sum L_{i}$. Consider the algorithm that at each
iteration samples $i$ with probability $L_{i}/L$ and sets $x^{(k+1)}\leftarrow x^{(k)}-\frac{1}{L_{i}}\frac{\partial}{\partial x_{i}}f(x^{(k)})e_{i}$.
Then, we have that 
\[
