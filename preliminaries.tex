...
\begin{defn}
A function $f$ is in $\mathcal{C}^{k}(\Rn)$ if $f$ is $k$-times
differentiable and its $k^{\mathrm{th}}$ derivative is continuous.
\end{defn}
...
at $x_{0}$ if and only if 

\[
{\displaystyle \liminf_{x\to x_{0}}f(x)\geq f(x_{0})}.
\]
The function is lower semi-continuous if it is so at every point in
...
\[
g(y)=\sum^{k}_{j=0}g^{(j)}(x)\frac{(y-x)^{j}}{j!}+g^{(k+1)}(\zeta)\frac{(y-x)^{k+1}}{(k+1)!}.
\]
\end{thm}

\begin{defn}
The \emph{convolution} of two functions $f,g:\R^{n}\rightarrow\R$
is defined as $h(x)=\int_{\R^{n}}f(z)g(x-z)\,dz.$
\end{defn}
...
\begin{defn}
For any matrix $A$, we define its trace, $\tr A=\sum_{i}A_{ii}$,
Frobenius norm, $\,\|A\|^{2}_{F}=\tr(A^{\top}A)=\sum_{i,j}A^{2}_{ij}$,
and operator norm, $\,\|A\|_{\op}=\sup_{\|x\|_{2}=1}\|Ax\|_{2}$.
Note that $x^{\top}Ax=\tr(Axx^{\top})$ and in general, $\tr(AB)=\tr(BA)$.
\end{defn}

For symmetric $A$, we have $\tr A=\sum_{i}\lambda_{i}$, $\|A\|^{2}_{F}=\sum_{i}\lambda^{2}_{i}$
and $\|A\|_{\op}=\max_{i}|\lambda_{i}|$ where $\lambda_{i}$ are
the eigenvalues of $A$.

For a vector $x$ and a positive definite matrix $A$, $\norm x_{A}=\sqrt{x^{T}Ax}$;
for a matrix $B$, 
\[
\norm B_{A}=\sup_{x\ne0}\frac{\norm{Bx}_{A}}{\norm x_{A}}.
\]
...
\]
provided $A$ is invertible and $1+u^{\top}A^{-1}v\neq0$. For matrices
$U,V\in\R^{n\times k},$ we have, provided $I+V^{\top}A^{-1}U$ is
invertible,
\[
...
\]
Here, $\rho(S)$ denotes the measure induced by the density $\rho$,
i.e., $\rho(S)=\int_{S}\rho(x)\,dx$, and similarly for $\nu$. The
last equality follows because $\rho-\nu$ has total mass zero.
\end{defn}
...
\begin{defn}
The $\chi$-squared distance of a density $\rho$ with respect to
another density $\nu$ is defined as  
\[
...
is defined as follows: for a convex function $f:\R_{+}\to\R$ with
$f(1)=0$ and $f'(\infty)=\infty$, 
\[
...
for $1\leq q\leq q'$ and $2\,\norm{\cdot}^{2}_{\tv}\leq\KL\leq\RR_{2}=\log(\chi^{2}+1)\leq\chi^{2}$. 
\end{fact}
...
onto a $k$-dimensional subspace $V$ is defined as 
\[
...
\section*{Geometry}
\begin{defn}
We denote the unit Euclidean ball by $B^{n}=\left\{ x:\norm x_{2}\le1\right\} .$
More generally, we define the $\ell_{p}$-norm ball of radius $r$
centered at $z$ as $B^{n}_{p}(z,r)=\left\{ x:\norm{x-z}_{p}\le r\right\} $.
\end{defn}
