...
only one part.
\item Taylor Approximation: $f(y)\approx\sum_{k=0}^{K}D^{k}f(x)[y-x]^{k}/k!$
.
...
Suppose (3) holds, then $g(x)=f(x)+\frac{L}{2}\|x\|_{2}^{2}$ satisfies
$g(y)\geq g(x)+\nabla g(x)^{\top}(y-x)$ for all $x,y\in\Rn$. So
Theorem \ref{thm:equ_convex} shows that $g$ is convex and $\nabla^{2}g(x)\succeq0$.
Hence, $\nabla^{2}f(x)\succeq-LI$. Similarly, by taking $g(x)=\frac{L}{2}\|x\|_{2}^{2}-f(x)$,
...
$f(x)-f(x^{*})=\|\nabla f(x)\|_{2}^{2}$ and hence both have the same
...
is used as a subroutine. We note that for the case $f(x)=\frac{1}{2}\|x\|_{2}^{2}$,
...
are equivalent:
\begin{itemize}
\item $f(tx+(1-t)y)\leq tf(x)+(1-t)f(y)-\frac{1}{2}\mu t(1-t)\|x-y\|_{2}^{2}$
...
Finally, we note that for problems of the form $f(x)=\sum_{i}f_{i}(a_{i}^{\top}x)$,
...
and $h(y)=\|y-x\|_{2}^{2}$. In this case, the algorithm is called
