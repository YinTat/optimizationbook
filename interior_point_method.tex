...
\]
We will explain the reason for choosing $\ln x$ in more detail later.
For now, we can think of it as a nice function that blows up to $\infty$
as $x$ approaches zero.

One can think of $-\ln x$ as giving a force from every constraint
$x\geq0$ to make sure $x\geq0$ is true. Since the gradient of $-\ln x$
blows up when $x=0$, when $x$ is close enough, the force is large
enough to counter the cost $c$. When $t\rightarrow0$, then the problem
...
Therefore, when $\varepsilon<\frac{1}{2}$, we have both $\|X^{-1}\delta_{x}\|_{\infty}$
...
Using this, 
\[
\text{LHS}=\sum_{i}\left(x^{\new}_{i}s^{\new}_{i}-t\right)^{2}=\sum_{i}(x_{i}s_{i}+x_{i}\delta_{s,i}+s_{i}\delta_{x,i}+\delta_{x,i}\delta_{s,i}-t)^{2}=\sum_{i}\delta^{2}_{x,i}\delta^{2}_{s,i}\leq\left((1+\varepsilon)t\right)^{2}\cdot\sum_{i}\left(\frac{\delta_{x,i}}{x_{i}}\right)^{2}\left(\frac{\delta_{s,i}}{s_{i}}\right)^{2}
\]
...
\end{align}
where the first step follows from the fact that the modified program
solves a minimization problem, the second step follows from the definition
of $\overline{x}\in\R^{n+2}$ (\ref{eq:another_ov_x_in_appendix})
and $\overline{c}\in\R^{n+2}$ (\ref{eq:another_ov_c_in_appendix}),
the last step follows from $x\in\R^{n}$ is an optimal solution in
...
Then, we have 
\begin{align}
\frac{\delta}{L}\cdot c^{\top}\overline{x}_{1:n}+\theta\leq\overline{\text{OPT}}+\delta^{2}\leq\frac{\delta}{LR}\cdot\text{OPT}+\delta^{2},\label{eq:eq1_in_appendix}
\end{align}
...
Note that 
\begin{align}
\frac{\delta}{L}c^{\top}\overline{x}_{1:n}\geq-\frac{\delta}{L}\|c\|_{\infty}\|\overline{x}_{1:n}\|_{1}\geq-\frac{\delta}{L}\|c\|_{\infty}(n+1)\geq-\delta(n+1),\label{eq:lower_bound_on_delta_L_c_ovx}
\end{align}
where the second step follows from the transformed constraint $1^{\top}_{n}\overline{x}_{1:n}+\tau=n+1$
and nonnegativity, and the last step follows from $\|c\|_{\infty}\leq L$.

We can upper bound the $\theta$ in the following sense, 
\begin{align}
\theta\leq\frac{\delta}{LR}\cdot\text{OPT}+\delta^{2}+\delta(n+1)\leq(2n+1)\delta+\delta^{2}\leq4n\delta\label{eq:theta_is_at_most_4n_delta}
\end{align}
...
\end{align*}
Using $\hat{x}=R\overline{x}_{1:n}\in\R^{n}$, we have 
\begin{align*}
...
\ma & -\ma & 0\\
1 & 0 & 1
\end{array}\right]$, $\overline{c}=(c,\widetilde{c},0)$, $\overline{b}=(b,\widetilde{b})$
...
$\overline{x}=(x^{+}_{c},x^{-}_{c},\overline{R})$. $\overline{s}=t/\overline{x}$
componentwise. 
