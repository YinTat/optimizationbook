...
For $t>0$, if the polytope $\{Ax=b,x\geq0\}$ has an interior, then
the optimum of $(\mathrm{P}_{t})$ is unique and is given by the solution
...
\]
Note that for any primal-feasible $x$ and dual-feasible $(y,s)$,
we have that
\[
...
approximation of the primal solution. In fact, $nt$ itself is a bound
on the duality gap of the current solution.
...
\label{lem:primal_dual_representation}Suppose $\PriR$ has non-empty
interior and is bounded. For any positive vector $\mu\in\R^{n}_{>0}$,
...
with $x_{\mu}s_{\mu}=\mu$ for some $s_{\mu}$. Since $\PriR$ has
non-empty interior and is bounded and since $f_{\mu}$ is strictly
convex, such an $x_{\mu}$ exists. Furthermore, since $f_{\mu}(x)\rightarrow+\infty$
...
\begin{enumerate}
\item Inner radius $r$: There exists a point $x\in\PriR$ such that $x_{i}\geq r$
for all $i\in[n]$.
...
with inner radius $r$, outer radius $R$ and Lipschitz constant $L$.
For any $0<\epsilon\leq\frac{1}{2}$, the modified linear program
...
\]
Similarly, we have $\widetilde{b}=\sum_{i}x^{+}_{c,i}+\overline{R}\geq\frac{3}{4}n\overline{R}$. 
...
0 & \leq\frac{\mu^{\theta}}{x^{\theta}}(x^{\theta}-v_{\theta})+\sum^{n}_{i=1}(c_{i}-\frac{\mu^{+}_{i}}{x^{+}_{i}})(v^{+}-x^{+})_{i}+\sum^{n}_{i=1}(\widetilde{c}_{i}-\frac{\mu^{-}_{i}}{x^{-}_{i}})(v^{-}-x^{-})_{i}\\
 ...
 & \leq5nL\overline{R}-\frac{LRr}{2\min_{i}x^{+}_{i}}-\frac{t}{4\overline{R}}\max_{i}x^{-}_{i}.
\end{align*}
