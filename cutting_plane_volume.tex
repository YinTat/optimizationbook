...
a number $R$ such that $B_{n}\subseteq K\subseteq RB_{n}$ and $\varepsilon>0$,
the problem is to output a number $A$ such that 
\[
...
and $\varepsilon>0$, output a number $A$ such that 
\[
(1-\varepsilon)\vol(K)\le A\le(1+\varepsilon)\vol(K).
\]
...
we have that 
\[
\conv(x_{1},\ldots,x_{m})\subseteq K\subseteq B^{n}.
\]
By the following lemma, the volume of the convex hull of \emph{any
$m$ }points in the unit ball is at most a $m/2^{n}$ fraction of
the unit ball's volume. This proves the theorem.
\end{proof}
\begin{lem}
Let $x_{1},\ldots,x_{m}\in B^{n}$. Then 
\[
\vol\left(\conv(x_{1},\ldots,x_{m})\right)\le\frac{m}{2^{n}}\vol(B^{n}).
\]
\end{lem}

\begin{proof}
For each $x_{i}$, define $B_{i}=B(x_{i}/2,\|x_{i}\|/2)$. Then 
\[
\vol\left(\bigcup^{m}_{i=1}B_{i}\right)\le m\cdot\vol(B(0,1/2))=\frac{m}{2^{n}}\vol(B^{n}).
\]

We argue that $\conv(x_{1},\ldots,x_{m})\subset\bigcup B_{i}$. To
see this, let $y$ be a point not contained in any $B_{i}$. Then,
for every $i$, 
\[
\|y-x_{i}/2\|>\|x_{i}\|/2,
\]
...
\]
In other words, all the $x_{i}$ lie in the open halfspace $\{x:\langle y,x-y\rangle<0\}$,
while $y$ lies on its boundary. Therefore the hyperplane $\{x:\langle y,x-y\rangle=0\}$
separates $y$ from $\conv(x_{1},\ldots,x_{m})$. 
\end{proof}
...
Return $V_{\text{last}}\cdot\prod^{n}_{i=1}w_{i}(K^{(k)})$ where
...
the two sides passes through the original centroid (see Figure \ref{fig:cutting_plane_vol});
...
of Theorem \ref{thm:Grunbaum}, we may assume $K$ has rotationally
symmetric cross sections by replacing each cross section $\{x\in K\colon\theta^{\top}x=t\}$
with the $(n-1)$-dimensional ball of the same volume with radius
$r(t)$, i.e., $r^{n-1}(t)C_{n-1}$ is the $(n-1)$-dimensional volume
of $K\cap\{y\in\R^{n}:y^{\top}\theta=t\}$, with the center of gravity
at $t=0$. We will exploit the fact that $r(t)$ is concave in $t$,
and replace $r(t)$ by a line going through $(b,0)$ and $(0,r(0))$
so that we get a cone with base at $t=a$. We will show that the center
of gravity of the cone, say $t_{0}$, is to the left of $0$. Then,
\[
...
y(t)=-\frac{r(0)}{b}(t-b).
\]
Notice that $y(t)\le r(t)$ for all $t\in[0,b]$ because it is the
chord between two points of a concave function and therefore lies
below the concave function. To see why $r(t)\le y(t)$ for $t\in[a,0]$,
...
\begin{align*}
t_{0} & =\frac{\int^{b}_{a}ty^{n-1}(t)dt}{\int^{b}_{a}y^{n-1}(t)dt}\\
 & =\frac{1}{\int^{b}_{a}y^{n-1}(t)dt}\left(\int^{0}_{a}ty^{n-1}(t)dt+\int^{b}_{0}ty^{n-1}(t)dt\right)\\
 & \le\frac{1}{\int^{b}_{a}y^{n-1}(t)dt}\left(\int^{0}_{a}tr^{n-1}(t)dt+\int^{b}_{0}tr^{n-1}(t)dt\right)=0.
\end{align*}
So $t_{0}\le0$.
\end{proof}
...
$w_{i}(K^{(k)})\leq r$), the algorithm stops cutting along that axis.
So, just before the last cut along any axis, the width in that direction
is at least $r$. Then, we use the center of gravity to cut. By Lemma
\ref{lem:width}, the directional width along every axis of the surviving
set is at least $r/(n+1)$. Since the set always contains the origin,
and the original set contains a cube of side length $r$, when the
algorithm stops, it must be an axis-parallel cuboid with each side
of length in the range $[r/(n+1),r]$. So the final volume is at least
$(r/(n+1))^{n}.$ The initial volume is at most $(2R)^{n}$. Therefore
the number of iterations is at most 
\[
\log_{1/(1-\frac{1}{e})}\left(\frac{(2R)^{n}}{(r/(n+1))^{n}}\right)=O\left(n\log(nR/r)\right).
\]
...
\[
A=\prod^{n}_{i=1}[0,a_{i}]\ \mbox{and \ }B=\prod^{n}_{i=1}[0,b_{i}]
\]
for positive real numbers $a_{1},\ldots,a_{n},b_{1},\ldots,b_{n}$.
Then, we have 
\[
\vol(A)=\prod^{n}_{i=1}a_{i},\,\vol(B)=\prod^{n}_{i=1}b_{i},\quad\vol(A+B)=\prod^{n}_{i=1}(a_{i}+b_{i}).
\]
Now we need to verify that 
\[
\left(\prod^{n}_{i=1}(a_{i}+b_{i})\right)^{\frac{1}{n}}\ge\left(\prod^{n}_{i=1}a_{i}\right)^{\frac{1}{n}}+\left(\prod^{n}_{i=1}b_{i}\right)^{\frac{1}{n}}
\]
which is equivalent to 
\[
\left(\prod^{n}_{i=1}\frac{a_{i}}{a_{i}+b_{i}}\right)^{\frac{1}{n}}+\left(\prod^{n}_{i=1}\frac{b_{i}}{a_{i}+b_{i}}\right)^{\frac{1}{n}}\le1.
\]
Using the AM-GM inequality, for each of the terms on the LHS, we have
\[
\left(\prod^{n}_{i=1}\frac{a_{i}}{a_{i}+b_{i}}\right)^{\frac{1}{n}}+\left(\prod^{n}_{i=1}\frac{b_{i}}{a_{i}+b_{i}}\right)^{\frac{1}{n}}\le\frac{1}{n}\sum^{n}_{i=1}\frac{a_{i}}{a_{i}+b_{i}}+\frac{1}{n}\sum^{n}_{i=1}\frac{b_{i}}{a_{i}+b_{i}}=1.
\]

We will now prove the inequality for any finite union of cuboids.
That suffices to complete the proof because any measurable subset
is the limit of a union of disjoint cubes as the cubes become smaller
and smaller. 
