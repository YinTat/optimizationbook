...
Return $V_{\text{last}}\cdot\prod^{n}_{i=1}w_{i}(K^{(k+1)})$ where
...
$\max_{x\in K}\left|e^{\top}_{i}x\right|\leq r$), the algorithm stops
...
is at most $(2R)^{n}$. Therefore the number of iterations is at most
\[
